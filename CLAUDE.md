...
- `testahil-answer-challenger` — interrogates a study's ANSWER the way the principal does and
  does not stop at the study's own explanation, hunting OUR defect before conceding the market
  is wrong and pricing every candidate [R-GAP-01] [R-GAP-02]. It never edits the study.

**What to say to reach each one → `docs/agent_card/`** — a two-page card (what to say, what
comes back, where each stops) plus a schematic of where each agent sits. The `.docx` is
GENERATED by `build_card_docx.js`; edit the generator, never the document. It counts the
agents, so a new agent means a new row — that file's README says what to update.
