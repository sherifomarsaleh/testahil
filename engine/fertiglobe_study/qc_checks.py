...
STUDY = os.path.join(HERE, 'Fertiglobe_Valuation_Study_09-08-2026.docx')
BIB = os.path.join(HERE, 'Fertiglobe_Bibliography_09-08-2026.docx')
...
for label, path in [(l,p) for l,p in [('study', STUDY), ('bibliography', BIB)] if os.path.exists(p)]:
    _, txt = doc_text(path)
    low = txt.lower()
    hits = []
...
for label, path, maxw in [(l,p,w) for l,p,w in [('study', STUDY, TEXT_WIDTH), ('bibliography', BIB, BIB_WIDTH)] if os.path.exists(p)]:
    d = Document(path)
    over, narrow, wide = [], [], []
