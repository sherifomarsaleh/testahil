#!/usr/bin/env python3
"""FERTIGLOBE — every total a reader sees, reproduced from the rows printed above it.
...
DOCS = [d for d in (_latest('Fertiglobe_Valuation_Study_*.docx'), _latest('Fertiglobe_Bibliography_*.docx')) if d]

DECLARED = [
    (_latest('Fertiglobe_Valuation_Study_*.docx'), 32, "Total assets",
     "A CONDENSED LAYOUT against a disclosed total: between 15.5% and 58.2% of assets are "
     "not broken out in the rows above, the widest of any study in this book, and the "
     "half-year column is the widest of its own columns."),
    (_latest('Fertiglobe_Valuation_Study_*.docx'), 32, "Total equity",
     "A DISCLOSED LINE ITEM printed after the liability lines, not a roll-up of them."),
    (_latest('Fertiglobe_Valuation_Study_*.docx'), 14, "Weighted average cost of capital",
     "Transposed cost-of-capital table: its weights are a ROW, so the figure is "
     "reproducible by a reader and not column-wise."),
    (_latest('Fertiglobe_Valuation_Study_*.docx'), 15, "Weighted average capitalisation rate",
     "A capitalisation rate blended across the asset base, not a sum of the rows above."),
]

if __name__ == '__main__':
    examined, problems = TF.check(DOCS, DECLARED)
    assert examined, 'no tables examined — an empty result is not a clean result'
    sys.exit(TF.report(examined, problems, 'FERTIGLOBE'))
