...
def run():
    ...
    res = dict(
        ...
        n_cells=len(rows),
        by_driver=summarise(rows),
        by_driver_horizon=summarise(rows, key=lambda r: "%s@h%d" % (r["driver"], r["horizon"])),
        by_horizon=summarise(rows, key=lambda r: "h%d" % r["horizon"]),
...
if __name__ == "__main__":
    ...
    print("\nmacro / company split:")
    for d, v in r["macro_split"].items():
