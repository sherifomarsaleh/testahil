...
# [R-DOC-01] resolved by pattern — the digest is named for its latest amendment day
import glob as _glob
_digests = sorted(_glob.glob(os.path.join(os.path.dirname(HERE), 'PROJECT_INSTRUCTIONS_*.md')))
...
<footer>
  Generated from <code>engine/{os.path.basename(SRC)}</code>. Regenerated by
