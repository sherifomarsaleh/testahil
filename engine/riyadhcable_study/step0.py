"""Riyadh Cables study — Step 0.0 data-quality gate + Step 0 calibration gate.

Production-identical: the live SA fit is read from engine/fitted_configs.json
...
blocks {2,3,4}). RIYADHCABLE is NEW coverage: it has no committed fitted_configs
entry, so the reconciliation ASSERT is on the FIT ITSELF — the (nu, width_cal)
used must be exactly the committed SA production fit, and the name must not
already sit in the panel.
"""
import sys, os, json
...
assert (NU, CAL) == (float(SA.nu), float(SA.width_cal)), \
    f"registry ({NU},{CAL}) != profile ({SA.nu},{SA.width_cal}) — mirror out of sync"
assert 'RIYADHCABLE' not in reg['panel_names'], "RIYADHCABLE already covered — not new coverage"

# ---- Step 0.0 — data-quality gate ----------------------------------------
df_raw = raw_load(os.path.join(HERE, '..', 'raw_ohlc', 'SA', 'RIYADHCABLE.csv'))
df, dq_log = clean_ohlc(df_raw, 'RIYADHCABLE', verbose=True, market='SA')
scr = screen(df)
...
print(f"Step 0.0: raw {len(df_raw)} rows -> clean {len(df)} rows | "
      ...
      f"(SA artifact threshold {jump_threshold('SA'):.3f}) | flat_frac {scr['flat_frac']:.3f}")

# ---- Step 0 — calibration gate (calendar 3m, carry-anchored, live SA fit) --
r = backtest_v3(df, SA, horizon_months=3, nu=NU, width_cal=CAL,
                use_signal=SA.signal_active, n_paths=20000, seed=42,
                min_history=260)
n_all = len(r)
r = apply_breaks(r, SA)
