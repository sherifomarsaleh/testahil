#!/usr/bin/env python3
"""SCEM — every total a reader sees, reproduced from the rows printed above it.
...
DOCS = [d for d in (_latest('SCEM_Valuation_Study_*.docx'), _latest('SCEM_Bibliography_*.docx')) if d]

DECLARED = [
    (_latest('SCEM_Valuation_Study_*.docx'), 16, "Weighted average cost of capital",
     "The cost of equity and the after-tax cost of debt weighted by the equity and debt "
     "weights, which this table prints as a ROW: it is transposed, its rows being line "
     "items and its columns the bases. Reproducible by a reader, not column-wise."),
]

if __name__ == '__main__':
    examined, problems = TF.check(DOCS, DECLARED)
    assert examined, 'no tables examined — an empty result is not a clean result'
    sys.exit(TF.report(examined, problems, 'SCEM'))
