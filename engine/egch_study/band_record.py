...
JS = ("const fs=require('fs');const vm=require('vm');const src=fs.readFileSync(%r,'utf8');"
      ...
      "console.log(JSON.stringify({n:b.n,hits:b.hits,c50:b.c50,c80:b.c80,c90:b.c90,strength:b.strength,flag:b.flag,width:b.width}));"
      % os.path.join(ROOT, 'assets', 'data.js'))
out = subprocess.run(['node', '-e', JS], capture_output=True, text=True, check=True).stdout
rec = json.loads(out)
assert rec['n'] > 0 and 0 < rec['c90'] <= 1 and rec['width'] > 0 and rec['strength']
json.dump(rec, open(os.path.join(HERE, 'band_record.json'), 'w'), indent=1)
print("band record: n=%d c50=%.4f c80=%.4f c90=%.4f width=%.3f strength=%s flag=%s" % (rec['n'], rec['c50'], rec['c80'], rec['c90'], rec['width'], rec['strength'], rec['flag']))
