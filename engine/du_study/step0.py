"""DU study — Step 0.0 data-quality gate + Step 0 calibration gate.
...
robust_verdict across bootstrap blocks {2,3,4}). DU is NEW coverage: it has
...
must match the committed registry (AE: PARITY — CI straddles zero narrowly on
the upside; the standing market-panel gate treats PASS and PARITY as proceed,
FAIL as stop).
"""
import sys, os, json
...
assert 'DU' not in reg['panel_names'], "DU already covered — not new coverage"

# ---- Step 0.0 — data-quality gate ----------------------------------------
df_raw = raw_load(os.path.join(HERE, '../raw_ohlc/AE/DU.csv'))
df, dq_log = clean_ohlc(df_raw, 'DU', verbose=True, market='AE')
scr = screen(df)
