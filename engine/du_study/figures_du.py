"""DU study figures. House palette; SOLID light background on every figure so the
...
spot = d['spot']
df, _ = clean_ohlc(load_ohlc(os.path.join(HERE, '..', 'raw_ohlc', 'AE', 'DU.csv')),
...
ax.set_title('du — valuation football field (bear–bull span per lens; brass tick = base)',
             fontsize=10, pad=10)
...
ax.plot(s.index, s.values, color=INK, lw=1.7, label='DU close')
...
SEGLAB = dict(mobile='Mobile', fixed='Fixed', wholesale='Wholesale', ict='ICT & assoc.')
SEGCOL = dict(mobile=SAGE, fixed=GOLD, wholesale='#7B8D88', ict=BRASS)
segdata = {s: [d['seg_fy25']['rev'][s]] + F['seg_rev'][s] for s in SEGS}
...
for s_ in SEGS:
    ...
    ax.bar(xs, vals, width=0.56, bottom=bottom, color=SEGCOL[s_],
           alpha=0.85 if s_ != 'mobile' else 0.75, label=SEGLAB[s_],
           edgecolor='#FFFFFF', linewidth=0.6)
    bottom += vals
for i in range(len(yrs)):
    ax.text(i, bottom[i] + 0.15, f'{bottom[i]:.1f}', ha='center', fontsize=8.4, color=INK)
...
for i, m in enumerate(mar):
    ax2.text(i, m * 100 - 0.42, f'{m*100:.1f}%', ha='center', fontsize=8.2, color=INK,
...
# keep the margin line in the middle band of the chart, clear of the bar-total labels
ax2.set_ylim(min(mar) * 100 - 0.8, max(mar) * 100 + 3.2)
ax2.grid(False)
...
for i, (nm, ba, (lo, hi)) in enumerate(ex):
    ...
    ax.text(hi + 0.02 * xr, y, f'{lo:.1f}–{hi:.1f} · base {ba:.1f}', va='center', fontsize=8.6)
ax.axvline(spot, color=INK, lw=1.6)
