...
const children = [
  ...
  H("Sum of the parts — one table, every step", 1),
  P("GB Corp is three businesses in one listing: an automotive assembler-distributor, a non-bank lender, and a " + f2(100 * mnt.stake_current) + "% stake in MNT-Halan (" + f2(100 * mnt.stake_post) + "% once the announced second closing completes). The table below shows every scenario as one column, top to bottom: value each piece, add them up, state it per share, apply the 10% holding-company discount (conglomerates usually trade below the sum of their pieces), and state the total per share. The two Base columns differ in ONE thing — what the MNT-Halan stake is worth: its audited book value (framing A) or the price Al Ahly Capital actually paid in the June 2026 funding round (framing B). We show both and never average them; the update's central case stands on the round price, the bear case on book."),
  (() => { const R = N.sotp_recon; const col = (c, k, dec) => dec ? f1(c[k]) : f0(c[k]);
    return table(["EGP mn (parent share)", "Bear", "Base — stake at book (A)", "Base — stake at round price (B)", "Full"],
    [["Car business (GB Auto, DCF)", f0(R.bear.auto), f0(R.base_book.auto), f0(R.base_round.auto), f0(R.full.auto)],
     ["Lender (GB Capital ex-MNT)", f0(R.bear.cap), f0(R.base_book.cap), f0(R.base_round.cap), f0(R.full.cap)],
     ["MNT-Halan stake", f0(R.bear.mnt), f0(R.base_book.mnt), f0(R.base_round.mnt), f0(R.full.mnt)],
     ["Other investments", f0(R.bear.other), f0(R.base_book.other), f0(R.base_round.other), f0(R.full.other)],
     ["Sum of the pieces", f0(R.bear.sum), f0(R.base_book.sum), f0(R.base_round.sum), f0(R.full.sum)],
     ["= per share, BEFORE discount", f1(R.bear.sum_ps), f1(R.base_book.sum_ps), f1(R.base_round.sum_ps), f1(R.full.sum_ps)],
     ["10% holding discount", f0(R.bear.discount), f0(R.base_book.discount), f0(R.base_round.discount), f0(R.full.discount)],
     ["Total after discount", f0(R.bear.total), f0(R.base_book.total), f0(R.base_round.total), f0(R.full.total)],
     ["= per share, AFTER discount", f1(R.bear.total_ps), f1(R.base_book.total_ps), f1(R.base_round.total_ps), f1(R.full.total_ps)]],
    [3100, 1650, 1900, 2000, 1650]); })(),
  P("", { after: 60 }),
  P("Column notes: Bear = framing A plus the operating legs stressed (car business at 60% of its DCF value, lender at 0.8× book). Full = operating legs flexed up (×1.4) with the stake at the post-second-closing round value grown to a next mark. The lens table below reads straight off this one: its 'Sum of the parts' row is the AFTER-discount line (Bear / Base-B / Full columns) and its 'before the discount' row is the BEFORE line (Bear column uses Base-A's before-discount figure, since the bear framing is the book value).", { color: MUTED }),
  P("The gap between the two Base columns — about " + f1((so.total_round_framing - so.total_book_framing) * 0.9 / N.shares_mn) + " EGP per share — is the honest width of this valuation. Framing B is a real transaction price; framing A is the audited book, but the auditor qualifies it every period because MNT-Halan's own statements are not made available. In dollars, the market capitalisation (~USD " + f0(N.usd_cross_check.mktcap_usd_mn) + "mn) is close to the stake's round-mark value alone (~USD " + f0(N.usd_cross_check.mnt_stake_usd_mn) + "mn) — at the market price you are paying for the stake and getting the automotive and lending businesses at little value."),

  H("Four lenses, one field", 1),
  table(["Lens (EGP/share)", "Bear", "Base", "Full", "Weight"],
    [["Sum of the parts (after 10% discount)", f1(L.sotp.bear), f1(L.sotp.base), f1(L.sotp.bull), pc(L.weights.sotp, 0)],
     ["Sum of the parts (before the discount)", f1(L.prediscount.bear), f1(L.prediscount.base), f1(L.prediscount.full), pc(L.weights.prediscount, 0)],
