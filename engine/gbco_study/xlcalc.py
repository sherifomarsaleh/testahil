...
# SUMPRODUCT is matched FIRST and separately: it is the one function here whose arguments
# must stay positionally aligned, so it cannot go through the numeric-filtering path the
# aggregate functions use. It is matched before FUNC because "SUM" is a prefix of it.
SUMPROD = re.compile(r'\bSUMPRODUCT\(([^()]*)\)')
...
class Book:
    ...
    def range_values_aligned(self, sheet, rng):
        """Positionally aligned values — non-numeric cells become 0.0 rather than vanishing."""
        c1, r1, c2, r2 = range_boundaries(rng)
        out = []
        for rr in range(r1, r2 + 1):
            for cc in range(c1, c2 + 1):
                v = self.cell_value(sheet, f'{get_column_letter(cc)}{rr}')
                out.append(float(v) if isinstance(v, (int, float)) else 0.0)
        return out

    def one_range(self, arg, sheet):
        arg = arg.strip()
        if not RANGE.match(arg):
            raise ValueError(f'SUMPRODUCT argument is not a range: {arg!r}')
        tgt, rng = sheet, arg
        sm = re.match(r"(?:'([^']+)'|([A-Za-z][A-Za-z0-9_]*))!(.+)", arg)
        if sm:
            tgt = sm.group(1) or sm.group(2); rng = sm.group(3)
        return self.range_values_aligned(tgt, rng.replace('$', ''))
    ...
    def evaluate(self, expr, sheet):
        ...
        while True:
            ...
            if len(parts) < 2:
                raise ValueError(f'SUMPRODUCT needs at least two ranges: {m.group(0)!r}')
            cols = [self.one_range(p, sheet) for p in parts]
            n = len(cols[0])
            if any(len(c) != n for c in cols):
                raise ValueError(f'SUMPRODUCT ranges differ in length: {m.group(0)!r}')
            total = 0.0
            for k in range(n):
                prod = 1.0
                for c in cols:
                    prod *= c[k]
                total += prod
            e = e[:m.start()] + repr(float(total)) + e[m.end():]
