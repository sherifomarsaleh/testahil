...
w("## A FALSIFIER FIRED, WAS TESTED, AND THE JUDGEMENT SURVIVED\n")
_wc = next(j for j in cj['judgements'] if 'working-capital' in j['name'])
w("The working-capital judgement's own stated overturning condition reads:\n")
w(f"> {_wc['overturned_by']}\n")
w("**That condition is now met — by five disclosed quarters, not two.** GB Corp publishes the")
w("components quarterly in its own segment table, and the cycle above is built from them. The")
w("judgement was tested against its own falsifier and **survived**: the committed ladder is")
w("conservative against the latest reviewed period rather than aggressive. This is recorded")
w("because a falsifier that is never tested is a habit, and one that is tested and passes is the")
w("only kind of evidence a judgement can earn.\n")
