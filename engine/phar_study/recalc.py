...
checks = [
    ...
    ('weighted centre — Frame A',
     V('Fundamental Valuation', f'B{find("Fundamental Valuation", "CENTRE — FRAME A (the cash-flow lens, unweighted)")}'),
     LN['centre_A']),
    ('weighted centre — Frame B',
     V('Fundamental Valuation', f'B{find("Fundamental Valuation", "CENTRE — FRAME B (the cash-flow lens, unweighted)")}'),
     LN['centre_B']),
    ('summary sheet centre — Frame A',
     V('Summary', f'B{find("Summary", "CENTRE — FRAME A (the cash-flow lens, unweighted)")}'), LN['centre_A']),
    ('summary sheet centre — Frame B',
     V('Summary', f'B{find("Summary", "CENTRE — FRAME B (the cash-flow lens, unweighted)")}'), LN['centre_B']),
    # RE-DERIVED, NOT DELETED. "the weights sum to one" was the right test of a weighted
    # blend and is a test of something that no longer exists. The claim the architecture
    # actually makes is stronger: each frame's centre IS its own cash-flow read, to the
    # cent, and the retired blend is printed beside it as a memo rather than as an answer.
    ('the Frame A centre IS the Frame A cash-flow lens',
     V('Fundamental Valuation', f'B{find("Fundamental Valuation", "CENTRE — FRAME A (the cash-flow lens, unweighted)")}'),
     LN['centre_A']),
    ('the Frame B centre IS the Frame B cash-flow lens',
     V('Fundamental Valuation', f'B{find("Fundamental Valuation", "CENTRE — FRAME B (the cash-flow lens, unweighted)")}'),
     LN['centre_B']),
    ('the retired blend is printed as a memo and reproduces what it read',
     V('Fundamental Valuation', f'B{find("Fundamental Valuation", "Memo — the retired weighted blend this edition replaced, on Frame A")}'),
     LN['blend_A']),
