...
LESSONS = [
    ...
    L("L-288", "ALL", None,
      "AVERAGING TWO FRAMINGS OF A CONTESTED JUDGEMENT MANUFACTURES A DISAGREEMENT "
      "THAT NEITHER FRAMING HOLDS.",
      "When a study cannot settle a question, taking the middle looks like modesty. "
      "It is not: the midpoint is a position no reading of the evidence actually "
      "supports, and it hides the one thing a reader needs, which is that the answer "
      "turns on a decision somebody has to make. Published side by side, the two "
      "readings show the reader what the argument is about. Averaged, they produce a "
      "confident-looking number that is nobody's view.",
      "Fertiglobe's dual framing, read 04-Sep-2026",
      "build",
      "One study's cash-flow lens is the straight mean of two framings of its central "
      "contested judgement about its market. Against the latest known price the first "
      "framing sits 34.2% below and the second 1.8% below — one says the market is "
      "badly wrong and the other that it is right. Their mean sits 19.6% below, which "
      "is a disagreement with the market that NEITHER framing asserts, and it is the "
      "number the study publishes. Under the two-sided rule a study is held only if "
      "every branch breaches the limit, so the averaging also changes the study's own "
      "publication status.",
    ...
    L("L-320", "ALL", None,
      "An obstacle assessed from the SHAPE of a change is not the "
      "same as an obstacle measured, and the assessment is the "
      "cheaper thing to get wrong",
      "EMPOWER's terminal rebuild was built, run and priced in the "
      "model in under an hour — about +1.5% on the terminal and "
      "+0.5% on the answer — and was then DECLARED BLOCKED by the "
      "delivered workbook, which publishes four parallel live models "
      "on one sheet, each rebuilding the terminal in live formulas "
      "on a fixed row map built from chained range() allocations. "
      "The reasoning was that the sanctioned construction needs six "
      "more rows per block, and that inserting them moves addresses "
      "a dozen formulas on four other sheets name directly, which is "
      "the defect that broke five sheets on an earlier name [L-300]. "
      "EVERY ONE OF THOSE FACTS WAS TRUE AND THE CONCLUSION WAS "
      "WRONG. The full waterfall a reader is asked to follow has to "
      "appear ONCE — a parallel re-run owes the reader its RESULT, "
      "not a second copy of the derivation — so the four blocks each "
      "needed two figures rather than eleven rows, and every one of "
      "them already carried two rows the retired construction had "
      "left dead. The change was made the same day with no row "
      "inserted anywhere and no address moved. WHAT WENT WRONG WAS "
      "NOT THE CARE BUT THE ORDER: the cost was estimated from the "
      "structure of the builder instead of from the smallest change "
      "that would actually satisfy the rule, and an estimate of a "
      "construction is not the construction — the same error, in the "
      "same fortnight, that made DU's own review predict +5.6% where "
      "the built answer came out -2.6%. Where a rebuild looks "
      "blocked, the thing to measure is the MINIMUM change the rule "
      "demands, and it is almost always smaller than a faithful "
      "re-run of what is already there.",
      "EMPOWER re-strike, readability and terminal pass, 4 September "
      "2026",
      "build",
      "compute.py routed through the sanctioned module: terminal "
      "24,650 -> 25,030 (+1.5%), central 1.8388 -> 1.8523 (+0.7%), "
      "asset life 28.0984 years derived from the FY2025 property, "
      "plant and equipment note. build_xlsx_empower.py: the "
      "waterfall built once at DCF rows 92-111 in columns C and G; "
      "the crux, 15%, bear and upside blocks each re-run through "
      "_sanctioned_fcff() into two rows the retired construction had "
      "vacated. Row map S_PVEX..S_PS = range(40,51) unchanged, "
      "xlsx_expected.json re-asserted, recalc and driver_test clean. "
      "The superseded assessment is kept, marked as superseded, in "
      "engine/empower_study/TERMINAL_EVIDENCE_04-09-2026.md.",
      "A rebuild where the minimum change genuinely does require the "
      "rows — a builder whose parallel blocks are themselves what a "
      "reader is asked to follow, so the derivation cannot be shown "
      "once and referenced."),
    L("L-321", "ALL", None,
      "A review that declares a correction impossible is making a claim, "
      "and it gets measured like every other claim",
      "RIYADHCABLE's own gap review, written that morning, examined the "
      "terminal, found it on the retired reinvestment identity, and "
      "concluded that the correction could not be made honestly. It gave "
      "two reasons, argued carefully, both TRUE, and NEITHER CONCLUSION "
      "FOLLOWED. First, that the asset life could not be derived: the "
      "identity returns 61.9 years for a plant line the company "
      "depreciates over 20 to 30, so 'the derivation is not usable on "
      "this issuer'. The observation is exact and the inference is wrong "
      "— the identity does not return the ACCOUNTING life and is not "
      "trying to; it returns the ECONOMIC replacement cycle, and it runs "
      "longer for a reason disclosed in the same note, that SAR 841.7mn "
      "of cost, 29% of the depreciable base, is fully depreciated AND "
      "STILL IN USE. Second, that the growth correction was coupled to "
      "the construction one, because zeroing the terminal real growth "
      "would leave the last explicit year growing 6.2% against a 2.0% "
      "terminal. True of ZEROING it — and the rule requires a stated "
      "real growth to be WRITTEN DOWN, not zeroed, so storing the "
      "existing 4% nominal as +1.96% real reproduces it to the basis "
      "point and moves nothing. Both corrections shipped the same day, "
      "worth +2.0%. THE PATTERN IS THE SECOND ONE IN A SINGLE DAY "
      "[L-320] AND IT IS THE MORE DANGEROUS OF THE TWO SHAPES: a "
      "'registered, priced, deliberately not applied' finding reads as "
      "the method at its most disciplined — it names the defect, prices "
      "it, and declines to fit — which is exactly why nobody re-opens "
      "it.",
      "RIYADHCABLE terminal rebuild, 4 September 2026",
      "build",
      "The prior GAP_REVIEW_04-09-2026.md priced the correction at "
      "-16.2% and did not apply it; the built correction came out "
      "+1.97%, opposite in sign and a fifth of the size, because it had "
      "been priced as 'zero the growth' rather than as 'store the growth "
      "and rebuild the terminal'. Central 127.9054 -> 130.4249. The "
      "derivation validates on the one leg this issuer discloses a "
      "single life for: software at 14.43 years derived against a stated "
      "15, 3.8% apart.",
      "A review that declines a correction, is re-tested, and turns out "
      "to have been right — which would make the discipline of "
      "re-testing cheap rather than necessary."),

    L("L-322", "ALL", None,
      "Gross cost over the year's charge returns the ECONOMIC "
      "replacement cycle, not the accounting life, and the gap between "
      "them is itself disclosed",
      "The identity used to derive a weighted asset life keeps coming "
      "out LONGER than the lives the accounting-policies note discloses, "
      "and that has twice been read as the identity failing. It is not: "
      "under a stable base, assets older than their accounting life are "
      "still in the gross cost and charge nothing, so the ratio recovers "
      "the life over which the base actually turns over. That is the "
      "quantity a maintenance charge needs — how much must be spent each "
      "year to keep the plant intact — and using the accounting life "
      "instead would charge for replacing plant that is demonstrably "
      "still running. THE COMPANIES SAY SO THEMSELVES: the disclosure "
      "'assets with a cost of X, fully depreciated but still in use' is "
      "the direct evidence, and it is a standard note. Two independent "
      "cross-checks make the derivation auditable rather than merely "
      "arguable: run the identity on a leg where a SINGLE life is "
      "disclosed and see whether it reproduces it, and compare the prior "
      "year's columns. Where a company discloses only RANGES, the "
      "identity is not a way around the disclosure — it is the "
      "weighting the disclosure does not give, and it is labelled "
      "derived.",
      "RIYADHCABLE terminal rebuild, 4 September 2026",
      "build",
      "Note 9 gross cost less land and assets under construction "
      "2,903,588,021 over a charge of 76,616,111 = 37.90 years, against "
      "disclosed ranges of 20-30 for plant; note 9 also discloses SAR "
      "841.7mn of cost, 29.0% of the depreciable base, fully depreciated "
      "and still in use. Software: 80,879,539 over 5,603,548 = 14.43 "
      "years against a DISCLOSED 15. Blended across notes 9, 10 and 13: "
      "35.7565 years, the three charges summing to 85,395,637, which is "
      "exactly the depreciation and amortisation the model carries.",
      "A company with no fully-depreciated-still-in-use disclosure and a "
      "young base, where the identity should return something at or "
      "below the disclosed life and a longer answer would mean the "
      "reasoning here is wrong."),

    L("L-323", "ALL", None,
      "Under the sanctioned terminal, book depreciation stops being only "
      "an add-back and becomes the BASE of the replacement charge, so "
      "its sign in the model reverses",
      "Every DCF in this house has treated depreciation as a non-cash "
      "add-back: more of it raises free cash flow, and the driver test "
      "asserted that direction. Under the sanctioned terminal on a "
      "book-depreciation-escalated maintenance basis that is no longer "
      "the whole story — a unit more of book depreciation adds a unit to "
      "terminal cash flow AND charges (1+inflation)^(life/2) units of "
      "maintenance at current cost, which on a long-lived base is more "
      "than one. Where the terminal carries most of the value the net is "
      "NEGATIVE: a business that depreciates its plant harder is one "
      "whose plant costs more to keep intact, which is the correct "
      "reading and one the retired construction could not express at "
      "all, because its terminal ignored depreciation entirely. THE "
      "DRIVER TEST IS WHERE THIS SURFACES, and the discipline is to "
      "re-derive the expected sign rather than to flip it: an assertion "
      "whose direction was re-pointed without a reason recorded is an "
      "assertion that has been switched off.",
      "RIYADHCABLE terminal rebuild, 4 September 2026",
      "build",
      "Depreciation and amortisation over revenue +0.5pp moved the "
      "cash-flow lens -0.47 SAR/share where it previously raised it; the "
      "maintenance escalator is 1.4248 on a derived 35.76-year life and "
      "the terminal is 81.4% of enterprise value. The same pass "
      "re-derived the asset-life driver's sign for the same reason: the "
      "life enters as the AGE of the base, not as a divisor, so "
      "lengthening it while holding the charge fixed says the plant is "
      "older and costs more to replace.",
      "A maintenance basis of replacement cost divided by the life, "
      "where the life IS a divisor and both signs run the other way — "
      "which is why the basis a study adopts is recorded rather than "
      "assumed."),

    L("L-324", "ALL", None,
      "Where a base is older than half its own life, the sanctioned "
      "maintenance charge is the LIGHTER of two defensible readings, and "
      "the direction is disclosed rather than corrected",
      "The construction escalates book depreciation over HALF the useful "
      "life, as a proxy for the age of the average asset. That proxy is "
      "exact only on a base with uniform vintages, and the actual age is "
      "measurable from the same note: accumulated depreciation over the "
      "year's charge. Where the two disagree the disagreement is "
      "information about which way the terminal errs, and it is not "
      "symmetric across the book — on one name the base came out YOUNGER "
      "than uniform, making the sanctioned charge the conservative one, "
      "and on the next it came out OLDER, making it the generous one. "
      "NEITHER IS FIXED INSIDE THE STUDY: replacing the shared "
      "construction's own proxy one name at a time is a rule change made "
      "in the wrong place, and a study that quietly used a different "
      "escalator would be outside the sanctioned construction while "
      "appearing to be inside it. What a study owes is the MEASUREMENT "
      "and the PRICE of the alternative, so a reader can see that the "
      "conservatism is a property of the construction rather than a "
      "choice anyone made.",
      "RIYADHCABLE terminal rebuild, 4 September 2026",
      "build",
      "Accumulated depreciation 1,775,547,325 over a charge of "
      "76,616,111 = 23.17 years of average age, against the 17.88 that "
      "half a 35.76-year life implies — the base is 5.30 years older "
      "than uniform, which is the same fact as the "
      "fully-depreciated-and-still-in-use disclosure seen from the other "
      "side. On the measured age the escalator is 1.5824 rather than "
      "1.4248, maintenance +11.0%, and the answer falls 1.33% to 128.69. "
      "Recorded in the gap review, not applied.",
      "A measurement showing the half-life proxy is unbiased across a "
      "book of names rather than name-specific, which would make the "
      "disclosure unnecessary — or enough names running one way to "
      "justify amending the shared construction, which is the honest "
      "route to changing it."),

    L("L-325", "ALL", None,
      "The row map existed and three checks in the same study read past "
      "it — a published address is only worth what reads it",
      "A workbook builder here already wrote the rows it had put the "
      "headline figures on into its own expected-values file. Three "
      "checks in the same directory — the recalculation gate, the driver "
      "test and the QC gate — each hard-coded those addresses instead, "
      "and all three silently pointed at the wrong rows the moment the "
      "terminal gained a waterfall. The recalculation gate reported an "
      "enterprise value of -88 and a terminal share of 1,313.75, which "
      "is loud; the DRIVER TEST reported a base of -62.74 and then "
      "declared FOURTEEN drivers dead, which reads exactly like a broken "
      "model rather than a broken address; and the QC GATE printed a "
      "PASS with a cell reference that no longer meant anything, which "
      "is the dangerous one. THE LESSON IS NOT 'publish the row map' — "
      "it WAS published, in the same directory, by the same build. It is "
      "that publishing an address does nothing until something reads it, "
      "and that the check most likely to keep hard-coding is the one "
      "that only PRINTS, because nothing it says has to reconcile with "
      "anything.",
      "RIYADHCABLE terminal rebuild, 4 September 2026",
      "build",
      "xlsx_expected.json already carried anchors for ev, tv_share and "
      "dcf_ps; recalc.py named DCF!C41/C42/C51, driver_test.py named "
      "DCF!C51 and qc_gate.py named DCF!C42, all three beside it. The "
      "same QC gate typed '555 of 555 formula cells' against an actual "
      "551 and '18 drivers' against 20, and named SWDY as the pattern "
      "this study follows three weeks after SWDY was removed from the "
      "reference set outright. All repointed to read what the build "
      "publishes, and the gate now asserts the reference set no longer "
      "contains the name it was quoting.",
    ...
    L("L-328", "ALL", None,
      "Accumulated depreciation over the charge is the average age "
      "only where the charge is cost over life, and two DISCLOSED "
      "conditions break that",
      "The identity holds because accumulated equals age times "
      "charge, which needs the annual charge to be cost divided by "
      "life AND the accumulated column to be this group's own "
      "depreciation history. THREE DISCLOSED CONDITIONS BREAK THAT. "
      "Where assets are depreciated to a RESIDUAL VALUE the charge "
      "is (cost less residual) over life, so the same accumulated "
      "balance buys more years and the ratio OVERSTATES the age. "
      "Where a useful life has been REASSESSED mid-life the charge "
      "is not level and the identity is broken outright rather than "
      "merely biased. And where the base was ASSEMBLED BY BUSINESS "
      "COMBINATION the accumulated column carries an acquiree's "
      "history against a charge that only ever reflects the period "
      "since acquisition, and the ratio is not a number about "
      "anything. All three are disclosed, so all three are "
      "checkable before the number is used. AND THE DISAGREEMENT "
      "ANNOUNCES ITSELF BEFORE THE NOTE DOES: compare the identity's "
      "implied life — depreciable gross cost over the charge — with "
      "the life the disclosed class rates weight to, and a large gap "
      "IS the accounts saying the charge is not cost over life. THE "
      "CHECK IS CIRCULAR WHERE A STUDY DERIVED ITS LIFE BY THE SAME "
      "IDENTITY, which is most of them, and saying so is part of "
      "running it — what does the work there is separate evidence "
      ...
      "September 2026",
      "build",
      "AIRARABIA's identity-implied life came to 26.42 years against "
      "the 17.84 its own disclosed class lives weight to — 48% apart, "
      "which no rounding explains. Its policy note then says both "
      "things outright: depreciation writes off cost 'less their "
      "estimated residual values', and the group 'changed the "
      "estimated useful life applied to certain assets' during the "
      "year. The measured age of 14.32 years is therefore not that "
      "fleet's age, and applying it would have moved the study -9.7% "
      "on a terminal carrying 95% of enterprise value. Built, run, "
      ...
      "MODON is the third condition and it is not close: AED "
      "6,643,801 thousand of accumulated depreciation arrived through "
      "a 2024 business combination against that year's entire charge "
      "of 193,207 — THIRTY-FOUR TIMES IT — and the two acquisitions "
      "are 97% of the closing column. Its naive age reads 15.45 years "
      "against 0.46 net of the acquired history. The arithmetic "
      "cross-check DOES catch that one, at about -34%, as it catches "
      "AIRARABIA at +48%: it works where the adopted and implied lives "
      "came from different routes and fails where they did not.",
"A residual small enough to be immaterial, or a reassessment "
    ...
    L("L-349", "ALL", None,
      "A DIRECTIONAL CLAIM IS TESTABLE FROM THE RECORD, AND THE "
      "NAIVE TEST IS NOT",
      "\"X sits above the market price\" is a claim a study can be "
      "held to using only its own committed values and no model at "
      "all, which makes it look like an easy gate. MEASURED ACROSS "
      "TWENTY-THREE DELIVERED STUDIES it is not: a first draft "
      "flagged 11 of 119 such sentences, a second draft — re-pointed "
      "to skip probability reads, to skip a spot compared with "
      "itself, and to take the NEAREST figure rather than the last in "
      "a window — flagged 16 of 57, WORSE at 28.1%, and inspection "
      "put almost all of it on the instrument rather than on the "
      "book. It read \"30% above the market price\" as a value of "
      "3.0; it took the top of a printed range as the subject of a "
      "sentence about that range's bottom corner; it ran one table "
      "cell's figure into the next row's direction word. THE SUBJECT "
      "OF A DIRECTIONAL SENTENCE IS NOT RECOVERABLE FROM THE PAGE, "
      "which is the same residue the waterfall gate hit and has the "
      "same answer: the test moves to whoever knows, because the "
      "BUILDER knows which value the sentence is about and the page "
      "never says. WHAT THE SWEEP DID FIND, and no gate in this "
      "repository could see either, is two delivered studies whose "
      "lens-comparison prose contradicts their own committed record — "
      "every individual figure correct, the relationship between them "
      "wrong.",
      "A book-wide directional sweep run while auditing STC, "
      "5 September 2026",
      "build",
      "ARCC's section 4 states \"two of them sit ABOVE the market "
      "price\" against a committed spot of 77.00 and committed lens "
      "values of 66.53, 65.57 and 45.65 — ALL THREE BELOW — then "
      "lists three names under \"two\" and quotes 49.64 and 55.21, "
      "neither of which is in its committed lens set: a sentence "
      "written for the values that study held before it was "
      "re-struck. GBCO's football-field caption reads \"Central fair "
      "value ~EGP 35.7/share, 14% below spot\" where its own stated "
      "spot is 31.25, so the magnitude is right and the direction "
      "inverted. False-positive rates measured at 9.2% and 28.1% on "
      "two drafts of the detector.",
      "It is overturned by a detector that resolves a sentence's "
      "SUBJECT reliably from the page — the residue here is entirely "
      "that problem and not the arithmetic, which is trivial. It "
      "would also be overturned, in the direction that matters, by "
      "studies DECLARING their directional claims the way a waterfall "
      "declares its steps: the claim becomes checkable at build time "
      "and the page-side detector stops being needed. What would NOT "
    ...
    L("L-355", "ALL", None,
      "A probe that guesses what something is called returns a number, "
      "not an error — which is why the guess survives.",
      "Reading a record means agreeing with it about names: the key a "
      "field is under, the spelling of a label, the way a file is dated, "
      "which direction a ratio runs. Where a reader assumes one of those "
      "and the record uses another, nothing raises. The reader finds "
      "less than is there and prints the smaller figure, and that figure "
      "is indistinguishable from a measurement. Read what the record "
      "DECLARES about itself before inferring anything from its shape, "
      "and where a reader finds nothing, treat that as evidence about "
      "the reader first.",
      "ten first-attempt measurements across one working session, "
      "07-09-2026, every one wrong the same way",
      "build",
      "Measured on the session's own working: a directory excluded by "
      "substring swallowed .github and condemned two files that were on "
      "disk; a provenance field read under one of its two spellings "
      "reported 58% of 3,862 inputs missing something none of them "
      "lacked; an artefact grepped under one of its three names reported "
      "two breaches where there is one; a ratchet walked without "
      "distinguishing debt from its opposite inflated the programme's "
      "headline count 49 against 45; a band read for its key names "
      "rather than its own declared basis field reported an acceptance "
      "criterion MET that is NOT MET; and a document picker sorting "
      "DD-MM-YYYY strings lexicographically read superseded editions. "
      "NONE raised. Every one produced a plausible figure, and three of "
      "them were reported before being corrected.",
      "A session in which a first-attempt measurement is wrong for some "
      "OTHER reason — an arithmetic slip, a wrong population, a bad "
      "assumption about the world rather than about a name. The claim "
      "is about naming specifically; a different failure mode would say "
      "this one is not the dominant hazard it appeared to be."),

    L("L-356", "STOCK", "ELEC",
      "The forecasting method could not be tested on ELEC at all.",
      "A walk-forward needs an origin to project from and a later "
      "actual to score against. Where the archive is too short there "
      "is no test to run, and saying so is the result — not a gap to "
      "be filled by stretching the window.",
      "ELEC walk-forward, date not recorded",
      "walk_forward_fundamental",
      "4 sourceable fiscal years on the best available basis against "
      "a bar of 5, and 0 scoreable origins: the last sourceable "
      "actual is FY2023, so the last possible origin is FY2022 at "
      "h=1, and FY2022 needs five years of history to it "
      "(FY2018-FY2022) which do not exist on any basis in hand.",
      "A filing archive that becomes reachable, or filings supplied "
      "directly, taking the sourceable span to five years or more on "
      "one basis."),

    L("L-357", "ALL", None,
      "An index of filings is not an archive of filings.",
      "A company can list its statements on its own website and serve "
      "none of them. The list looks like evidence the documents exist "
      "and are obtainable; only fetching each one tells you which. "
      "Record the outcome per file, not per page.",
      "ELEC walk-forward, date not recorded",
      "walk_forward_fundamental",
      "61 statement files listed; 19 on the live host and 0 of those "
      "fetchable; 42 on a host whose DNS does not resolve; the "
      "index's last period is 2025-09-30 while the last statement on "
      "the basis the study models is FY2020.",
      "An index whose listings are shown to be reliably fetchable, "
      "making the per-file probe redundant."),

    L("L-358", "ALL", None,
      "A study's historicals can be unverifiable and still pass every "
      "gate.",
      "Where a company stops publishing on the basis a study models, "
      "the study's history quietly becomes a vendor's account of it. "
      "Every arithmetic check still passes, because the numbers are "
      "internally consistent — they are simply nobody's filed "
      "numbers. The test is to measure the study's panel against the "
      "filings that DO exist and against the wedge between the two "
      "bases, rather than to assume the difference is the "
      "consolidation.",
      "ELEC walk-forward, date not recorded",
      "walk_forward_fundamental",
      "At the one year existing on both bases the group is 1.73x the "
      "parent on revenue and 2.04x on net profit. The study's FY2023 "
      "revenue is 1.79x the parent — consistent with a consolidated "
      "figure — while its net profit is 3.09x, 1.51 times beyond what "
      "the measured wedge delivers.",
      "A study whose vendor panel is later reconciled line by line to "
      "filings on the same basis, showing the check adds nothing."),

    L("L-359", "ALL", None,
      "Check that a \"filed record\" was filed before reasoning from "
      "it.",
      "A review can reach the right conclusion about a study and "
      "still take its benchmark from the study's own inputs. Calling "
      "a number the company's own record makes it read as external "
      "evidence when it is the thing under test, and the argument "
      "then runs in a circle nobody can see.",
      "ELEC walk-forward, date not recorded",
      "walk_forward_fundamental",
      "A review described a terminal-margin range of 25.33%-30.68% as "
      "the company's own filed record; it is the study's committed "
      "hist_is, and two of its three years have no filing at all. The "
      "filed range on the basis that can be checked is 1.43%-17.49%, "
      "which puts the forecast of 12.30% INSIDE it rather than at "
      "half the lowest filed year, and puts the price's reverse read "
      "at 1.6 times the highest filed margin.",
      "A house convention that tags every committed historical with "
      "whether it came from a filing, making the confusion impossible "
      "to make."),

    L("L-360", "ALL", None,
      "The accounting-policies note usually gives a range, not a "
      "life.",
      "[R-TERM-01] needs one disclosed useful life and the policy "
      "note rarely supplies one. Reading a span and picking a point "
      "inside it is the choice the rule exists to forbid, so the "
      "honest output is the derived identity and a band.",
      "ELEC walk-forward, date not recorded",
      "walk_forward_fundamental",
      "Route (1) gave spans only (10-50, 4-25, 5-20, 5-20, 5, 5-10); "
      "no dominant class, no weighting. Route (2) — depreciable gross "
      "cost over the annual charge — gives 24.13 years on the full "
      "base and 17.54 excluding the 27.3% of that base the note "
      "itself discloses as fully depreciated and still in use, with a "
      "prior-year control at 24.99.",
      "A run of filings that do disclose a scalar or a dominant "
      "class, making route (1) the normal case rather than the "
      "exception."),

    L("L-361", "ALL", None,
      "A borrowing rate below the sovereign is the arithmetic "
      "failing, not the company borrowing cheaply.",
      "Dividing the finance charge by a liabilities total that "
      "includes trade payables, related-party balances, tax and "
      "provisions understates the rate. The useful part is that the "
      "error announces itself: a corporate cannot fund below its own "
      "government, so a rate under the sovereign is a denominator "
      "problem and can be caught without knowing the right answer.",
      "ELEC walk-forward, date not recorded",
      "walk_forward_fundamental",
      "On the borrowings that actually bear interest the rate runs "
      "8.49%, 8.98%, 18.25%; on total liabilities 4.48%, 5.88%, "
      "14.10% — understated by 3.09 to 4.15 points, and below the "
      "Egyptian sovereign in 2 of 3 years.",
    ...
    L("L-370", "ALL", None,
      "D11 depreciation forecasts run about 2.5 times too low for "
      "SWDY.",
      "The method misses this driver in the same direction almost "
      "every time, not at random. That is a fixable defect rather "
      "than noise — find what is wired wrong before adding any "
      ...
      "walk_forward_fundamental",
      "Bias -0.931 log (about 2.5 times too low), average miss 0.931, "
      "wrong in the same direction in 100% of cases, and the sign "
      "holds across every bootstrap block tested (n=30).",
      "A later run of the same name where the sign no longer holds "
      "across bootstrap blocks."),
    ...
    L("L-371", "ALL", None,
      "D15 finance costs forecasts run about 2.4 times too low for "
      "SWDY.",
      "The method misses this driver in the same direction almost "
      "every time, not at random. That is a fixable defect rather "
      "than noise — find what is wired wrong before adding any "
      ...
      "SWDY walk-forward, date not recorded",
      "walk_forward_fundamental",
      "Bias -0.880 log (about 2.4 times too low), average miss 0.919, "
      "wrong in the same direction in 91% of cases, and the sign "
      "holds across every bootstrap block tested (n=45).",
      "A later run of the same name where the sign no longer holds "
      "across bootstrap blocks."),

    L("L-372", "CLASS", "diversified industrial with a contracting arm",
      "D1 cable volume t forecasts run about 28% too high for SWDY.",
      "The method misses this driver in the same direction almost "
      "every time, not at random. That is a fixable defect rather "
      "than noise — find what is wired wrong before adding any "
      "correction factor.",
      "SWDY walk-forward, date not recorded",
      "walk_forward_fundamental",
      "Bias +0.250 log (about 28% too high), average miss 0.256, "
      "wrong in the same direction in 95% of cases, and the sign "
      "holds across every bootstrap block tested (n=20).",
      "A later run of the same name where the sign no longer holds "
      "across bootstrap blocks."),

    L("L-373", "CLASS", "diversified industrial with a contracting arm",
      "D5 contracting revenue forecasts run about 39% too low for "
      "SWDY.",
      "The method misses this driver in the same direction almost "
      "every time, not at random. That is a fixable defect rather "
      "than noise — find what is wired wrong before adding any "
      ...
      "SWDY walk-forward, date not recorded",
      "walk_forward_fundamental",
      "Bias -0.326 log (about 39% too low), average miss 0.390, wrong "
      "in the same direction in 76% of cases, and the sign holds "
      "across every bootstrap block tested (n=45).",
      "A later run of the same name where the sign no longer holds "
      "across bootstrap blocks."),

    L("L-374", "ALL", None,
      "The A gross profit error is the company, not the currency.",
      "Re-running every forecast with perfect foresight of inflation "
      "barely improves it. So devaluation is not the explanation, and "
      "looking for a macro fix would waste the effort.",
      "SWDY walk-forward, date not recorded",
    ...
    L("L-375", "ALL", None,
      "The A cost of revenue bias changes direction between regimes.",
      "It runs one way in one period and the other way in the next. "
      "Averaging them produces a correction that is wrong in both. "
      "Record it, do not correct for it.",
      "SWDY walk-forward, date not recorded",
      "walk_forward_fundamental",
      "By era: pre-float +0.026; first float -0.024; second float "
      "-0.460.",
      "A longer record in which one sign dominates across all "
      "regimes."),
