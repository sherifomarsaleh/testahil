"""MODON study — Step 0.0 data-quality gate + Step 0 calibration gate.
...
robust_verdict across bootstrap blocks {2,3,4}). MODON is NEW coverage: it has
...
must match the committed registry (PARITY at the 29-Jul-2026 fit; the pending
19-name refit that includes MODON moves the panel to PASS and sits in
engine/PENDING_REVIEW/AE_2026-08-09.md for human review)."""
import sys, os, json
...
assert 'MODON' not in reg['panel_names'], "MODON already covered — not new coverage"

# ---- Step 0.0 — data-quality gate ----------------------------------------
df_raw = raw_load(os.path.join(HERE, 'MODON_Stock_Price_History.csv'))
df, dq_log = clean_ohlc(df_raw, 'MODON', verbose=True, market='AE')
scr = screen(df)
...
summ = dict(
    ...
    pending_refit=dict(note="19-name AE refit including MODON, engine/PENDING_REVIEW/"
                            "AE_2026-08-09.md — market PASS +0.0099 CI[0.002,0.017], "
                            "MODON per-name PASS +0.0424 under LONO; width_cal 0.979 -> "
                            "0.972 proposed (-0.7% band move, under the 5% gate)"),
