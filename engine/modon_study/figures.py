"""MODON study figures. House palette; SOLID light background on every figure so the
...
spot = d['spot']
df, _ = clean_ohlc(load_ohlc(os.path.join(HERE, 'MODON_Stock_Price_History.csv')),
                   'MODON', verbose=False, market='AE')
...
ax.set_title('Modon Holding — valuation football field (bear–bull span per lens; brass tick = base)',
             fontsize=10, pad=10)
style(ax); fig.tight_layout(); fig.savefig(os.path.join(HERE, 'fig1_football.png')); plt.close(fig)

# ---- F2 sensitivity heatmap (cost of capital x terminal g) ------------------
...
ax.set_xlabel('terminal growth g'); ax.set_ylabel('cost of capital')
ax.set_title(f'DCF fair value (AED/share) — cost of capital × terminal growth; '
...
ax.plot(s.index, s.values, color=INK, lw=1.7, label='MODON close')
...
ax.set_title('Modon Holding — price against the moving-average stack, last 260 sessions',
             fontsize=10, pad=8)
...
ax.set_ylim(ymin - 0.06 * rng, ymax + 0.10 * rng)
# the fundamental central (3.71) sits ABOVE the whole three-month cone — say so
# in words rather than pinning the axis to an off-scale line
ax.text(days[-1] - 1, ymax + 0.075 * rng,
        f'fundamental central {cb:.2f} sits above this whole cone ↑',
        color=BRASS, fontsize=8.6, ha='right', va='top')
...
# ---- F7 segment mix and the EBITDA margin path ------------------------------
F = d['fcst']; yrs = ['FY25'] + [y.replace('E', '') for y in F['years']]
SEGK = [('red_rev', 'Real estate development', GOLD),
        ('ect_rev', 'Events, catering & tourism', SAGE),
        ('aim_rev', 'Asset & investment mgmt', BRASS),
        ('hosp_rev', 'Hospitality', '#7B8D88')]
s25 = d['seg_fy25']['rev']
hist0 = dict(red_rev=s25['red'], ect_rev=s25['ect'], aim_rev=s25['aim'],
             hosp_rev=s25['hosp'])
mar = [d['hist_is']['FY25']['ebitda'] / d['hist_is']['FY25']['rev']] + F['ebitda_margin']
fig, ax = plt.subplots(figsize=(9.7, 4.2), dpi=110)
xs = np.arange(len(yrs))
bottom = np.zeros(len(yrs))
for key, lab, col in SEGK:
    vals = np.array([hist0[key]] + F[key]) / 1000
    ax.bar(xs, vals, width=0.56, bottom=bottom, color=col, alpha=0.82, label=lab,
           edgecolor='#FFFFFF', linewidth=0.6)
    bottom += vals
for i in range(len(yrs)):
    ax.text(i, bottom[i] + 0.5, f'{bottom[i]:.1f}', ha='center', fontsize=8.4, color=INK)
...
for i, m in enumerate(mar):
    ax2.text(i, m * 100 - 0.9, f'{m*100:.1f}%', ha='center', fontsize=8.2, color=INK,
...
ax2.set_ylim(min(mar) * 100 - 3.2, max(mar) * 100 + 2.2)
ax2.grid(False)
h1, l1 = ax.get_legend_handles_labels(); h2, l2 = ax2.get_legend_handles_labels()
ax.legend(h1 + h2, l1 + l2, frameon=False, fontsize=8.2, loc='upper left', labelcolor=INK, ncol=2)
ax.set_title('Revenue by segment, and the EBITDA margin path', fontsize=10, pad=9)
style(ax)
