...
CASES = [
    ...
    ('Beta — adopted (own stock, five-year weekly vs the Abu Dhabi market index)', 'C', +0.20,
     'dcf', -1,
     'a higher beta raises the cost of equity and must lower the valuation'),
    ('Scenario: beta benchmark switch (0 = adopted index, 1 = cross-check index)', 'C', +1.0,
     'dcf', -1,
     'flipping to the cross-check beta (1.086, higher) must raise the cost of equity '
     'and lower the valuation — the published cross-check, live in the sheet'),
...
r_bsw = row_of('Scenario: beta benchmark switch (0 = adopted index, 1 = cross-check index)')
r_alt = row_of('Beta — cross-check (same regression vs the Dubai market index)')
...
assert on_bumped['dcf'] < on['dcf'] - 1e-6, (
    'with the benchmark switch on, a higher cross-check beta must lower the valuation')
r_adopt = row_of('Beta — adopted (own stock, five-year weekly vs the Abu Dhabi market index)')
