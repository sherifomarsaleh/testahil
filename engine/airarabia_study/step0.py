"""AIRARABIA study — Step 0.0 data-quality gate + Step 0 calibration gate.
...
robust_verdict across bootstrap blocks {2,3,4}). AIRARABIA is NEW coverage:
its raw CSV was placed at engine/raw_ohlc/AE/AIRARABIA.csv this session and the
full 19-name AE panel was refit with it (09-Aug-2026), so the registry ALREADY
carries an AIRARABIA per-name row — the reconciliation ASSERT here is that the
(nu, width_cal) used are exactly the committed AE production fit, that the
market-panel gate verdict matches the committed registry (PARITY — the standing
market gate), and that AIRARABIA did not arrive FAILING.
"""
import sys, os, json
...
assert reg['market_verdict'] == 'PARITY', reg['market_verdict']
assert 'AIRARABIA' in reg['panel_names'], "AIRARABIA missing from the refit AE panel"
assert 'FAIL' not in reg['per_name']['AIRARABIA']['verdict'], \
    f"AIRARABIA arrived FAILING: {reg['per_name']['AIRARABIA']}"

# ---- Step 0.0 — data-quality gate ----------------------------------------
df_raw = raw_load(os.path.join(HERE, 'AIRARABIA_Stock_Price_History.csv'))
df, dq_log = clean_ohlc(df_raw, 'AIRARABIA', verbose=True, market='AE')
scr = screen(df)
...
summ = dict(
    ...
    registry_per_name=reg['per_name']['AIRARABIA'],
