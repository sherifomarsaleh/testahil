"""SWDY study — Step 0.0 data-quality gate + Step 0 calibration gate.
...
robust_verdict across bootstrap blocks {2,3,4}). SWDY is NEW coverage: it has
...
# `assert 'SWDY' not in reg['panel_names'], "SWDY already covered -- not new coverage"`,
...
# the live production one. Now that SWDY sits in the panel the reconciliation is STRONGER
# than it was, because the name is inside the very panel (nu, width_cal) were fitted on.
_IN_PANEL = 'SWDY' in reg['panel_names']
print("Step 0 reconciliation: SWDY is %s the committed EG panel of %d names; the fit used "
