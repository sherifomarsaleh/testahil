#!/usr/bin/env python3
"""SWDY -- the BUILD ORDER for this study's numbers file, recorded because nothing
...
#   forecast_anchor.py   adds `forecast_anchor`          [R-ANCHOR-01]
#   bridge_record.py     adds `bridge_record`            [R-BRIDGE-01]
#   coc_record.py        adds `cost_of_capital_record`   [R-COC-01]
STEPS = ('compute.py', 'forecast_anchor.py', 'bridge_record.py', 'coc_record.py')
    ...
    for i, step in enumerate(STEPS, 1):
        ...
        if r.returncode != 0:
            ...
            print('FAILED at %s -- the numbers file is now in whatever state that step left '
                  ...
                  % step)
            return r.returncode
    print('all four steps ran in order; study_numbers.json carries every record')
    return 0
