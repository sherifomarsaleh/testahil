"""ADNOCLS — forward cone strike via the identical production chain
(Step 0.0 gate -> YZ proxy -> fit_har_v3 -> har_forecast_v3 -> carry_log_h ->
simulate_paths_v3, 50k paths, seed 42, live UAE fit), local CSV, no site write."""
import sys, os, json
...
Q_ANNUAL = 341.25 / (7398.498764 * 6.16 / 3.6725)   # declared FY2025 distribution of USD 325mn
# growing 5% a year from 2026 (FY2025 earnings release), over market capitalisation at the anchor close

prof = MP.PROFILES['AE']
raw = load_ohlc(os.path.join(HERE, 'ADNOCLS_Stock_Price_History.csv'))
df, rep = clean_ohlc(raw, 'ADNOCLS', verbose=False, market='AE')
