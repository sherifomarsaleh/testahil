#!/usr/bin/env python3
"""ADNOCLS — every total a reader sees, reproduced from the rows printed above it.
...
DOCS = [d for d in (_latest('ADNOCLS_Valuation_Study_*_public.docx'), _latest('ADNOCLS_Bibliography_*.docx')) if d]

DECLARED = [
    (_latest('ADNOCLS_Valuation_Study_*_public.docx'), 40, "Total assets",
     "A CONDENSED LAYOUT against a disclosed total: between 11.8% and 19.8% of assets are "
     "not broken out in the rows above, and the working-capital lines printed there are a "
     "breakdown of the net working capital row rather than peers of it."),
    (_latest('ADNOCLS_Valuation_Study_*_public.docx'), 12, "Blended enterprise multiple",
     "A multiple blended ACROSS PEERS, not a roll-up of the rows above it in this column."),
    (_latest('ADNOCLS_Valuation_Study_*_public.docx'), 12,
     "Blended price-to-earnings multiple, forward",
     "A multiple blended ACROSS PEERS, as above."),
]

if __name__ == '__main__':
    examined, problems = TF.check(DOCS, DECLARED)
    assert examined, 'no tables examined — an empty result is not a clean result'
    sys.exit(TF.report(examined, problems, 'ADNOCLS'))
