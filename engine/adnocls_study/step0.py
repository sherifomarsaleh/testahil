"""ADNOCLS study — Step 0.0 data-quality gate + Step 0 calibration gate.
...
robust_verdict across bootstrap blocks {2,3,4}). ADNOCLS is NEW coverage: it has
...
assert 'ADNOCLS' not in reg['panel_names'], "ADNOCLS already covered — not new coverage"

# ---- Step 0.0 — data-quality gate ----------------------------------------
df_raw = raw_load(os.path.join(HERE, 'ADNOCLS_Stock_Price_History.csv'))
df, dq_log = clean_ohlc(df_raw, 'ADNOCLS', verbose=True, market='AE')
scr = screen(df)
