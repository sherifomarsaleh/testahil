...
def apply_grade(src: str, row: dict, got: dict) -> str:
    ...
    #
    # A ROW IS GRADED INTO THE FIELDS IT CARRIES, AND NO OTHERS. Fourteen rows across
    # seven names (ADNOCLS, ADNOCDIST, ADNOCDRILL, DU, BOROUGE, SAVOLA, RIYADHCABLE)
    # were published by their studies' OWN first-coverage writers and carry
    # `realized_close:null` with NO realized_high / realized_low / in_90 / in_50 slots
    # beside it. The writer required the full triple, so every one of them raised
    # 'outcome block not in the expected shape' — and because that is a SystemExit, ONE
    # such row killed the entire sweep, including grades for every other name queued
    # behind it. Measured 07-Sep-2026: BOROUGE's 1-month cone resolved dead centre of
    # its own 50% band (realized quantile 0.500) and could not be recorded.
    #
    # This is the fix above one level deeper: there the LAYOUT decided whether a field
    # got written, here the FIELD SET does. The resolution is the one
    # scripts/check_grade_writer_layout.py already argued for when it named these rows —
    # THE MISSING FIELDS ARE NOT INVENTED, because inventing them would change the
    # structure of a published forecast row. Each block is written where the row has
    # somewhere to put it and skipped where it has not, so a row that never carried
    # in_90 is graded on its close alone rather than not graded at all. A block whose
    # field IS present but whose text does not match still raises: that is the layout
    # bug, and it has to stay loud.
    #
    # Rows carrying the full shape take the same branch they always did, so their output
    # is byte-identical — asserted by the replay negative control in sweep() and by
    # check_grade_writer_layout.py against the pre-fix writer.
    if _field(t, 'realized_close') is None:
        raise SystemExit('row carries no realized_close field — not a gradable row')

    # THE SPACE AFTER THE COLON IS NOT PART OF THE FIELD EITHER. Six of the fourteen
    # rows above (ADNOCLS, SAVOLA, RIYADHCABLE, both horizons) write `realized_close:
    # null` with a space, the house style of the builder that emitted them — so even
    # the widened field-set branch missed them. `\s*` around every colon closes the
    # last of it; the SEPARATOR EACH ROW USES IS PRESERVED rather than normalised, so
    # a row written in spaced style stays in spaced style and the diff is the grade
    # and nothing else.
    sep = lambda m: m.group(1)                       # the row's own ':' or ': '
    if _field(t, 'realized_high') is None and _field(t, 'realized_low') is None:
        # Abbreviated shape: the close is the only outcome slot this row has.
        old_outcome = re.search(r'realized_close(\s*:\s*)null,', t)
        if not old_outcome:
            raise SystemExit('outcome block not in the expected shape')
        t2 = t.replace(old_outcome.group(0),
                       f"realized_close{sep(old_outcome)}{num(got['realized_close'])},", 1)
    else:
        old_outcome = re.search(r'realized_close\s*:\s*null,\s+realized_high\s*:\s*null,'
                                r'\s+realized_low\s*:\s*null,', t)
        if not old_outcome:
            raise SystemExit('outcome block not in the expected shape')
        t2 = t.replace(old_outcome.group(0), new_outcome)

    # `realized_date` exists only on the abbreviated rows and no page reads it; it is
    # filled where present so the row does not record a close that arrived on no day.
    old_rd = re.search(r'realized_date(\s*:\s*)null', t2)
    if old_rd:
        t2 = t2.replace(old_rd.group(0),
                        f'realized_date{sep(old_rd)}"{got["_graded_on"]}"', 1)

    if _field(t2, 'in_90') is not None:
        old_stats = re.search(r'in_90\s*:\s*null,\s+in_50\s*:\s*null,'
                              r'\s+realized_quantile\s*:\s*null,\s+median_err\s*:\s*null,', t2)
        if not old_stats:
    ...
    # ...and on the same terms as the outcome block above, a row with no touch_hit
    # FIELD AT ALL has nowhere to put the ladder and is graded without it, while a row
    # that HAS the field and does not match still raises.
    old_th = re.search(r'touch_hit\s*:\s*(?:null|\{[^}]*\})', t2)
    if old_th:
        th = ', '.join(f'"{k}":{jb(got["touch_hit"][k])}' for k, _ in REL)
        t2 = t2.replace(old_th.group(0), 'touch_hit:{ ' + th + ' }')
    elif _field(t2, 'touch_hit') is not None:
