...
- **Measured 06-Sep-2026, correcting how the source documents put this — the gap is a LIVE number, not a stale one.** Three findings, in order:
  - **The fair values are dated.** 36 of the 37 EG names carry a published, dated valuation study (`files/{TK}_Valuation_Study_{DD-MM-YYYY}_public.pdf`; only CCAP has none), running 11-Jun-2026 (PHDC) to 09-Aug-2026 (PHAR). What is missing is the date in MACHINE-READABLE form — only 4 names carry `fairAsof` in `assets/data.js`, so a reader can see it and no gate can.
  - **A fair value is far less date-sensitive than the cone or the read, by construction.** The MC cone and the technical read re-anchor on every close; a DCF's output moves with earnings and cost of capital. So a June fair value is not stale in September the way a June cone would be, and CALENDAR AGE IS THE WRONG TEST. The right test is whether the study has seen the latest filing and the current rf — for the June/early-July strikes that means H1-2026 results and a moved CBE path, which the EG sliding WACC schedule consumes directly [R-FCAL-01 point-in-time discipline; the sliding-schedule clause].
  - **The price leg is refreshed monthly by the roll-forward, so the comparison is current.** Measured across the 37: median recorded-spot age 14 days, min 5, max 32; seven names sit ~a month back (SWDY, ELEC 5-Aug; PHAR, EGCH, SCEM, ARCC, AMOC 6-Aug), and four of those seven are exactly the four carrying `fairAsof` — newest value against the oldest price, the freshness inverted.
  - **Consequence for the ranking:** this channel is NOT blocked on data currency, as A and B's "undated" framing implies. It is computable today. What blocks it is the two items below — no graded record, and the construction standard of the older studies.
