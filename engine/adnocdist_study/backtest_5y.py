"""ADNOCDIST — walk-forward backtest reported over three window sets, so the
...
df, _ = clean_ohlc(load_ohlc(os.path.join(HERE, 'ADNOCDIST_Stock_Price_History.csv')),
                   'ADNOCDIST', verbose=False, market='AE')
...
# ---- 1-month horizon, same three window sets (the monthly metronome horizon)
r1 = backtest_v3(df, AE, horizon_months=1, nu=NU, width_cal=CAL,
                 use_signal=AE.signal_active, n_paths=20000, seed=42, min_history=260)
res['h1_full'] = score(r1, '1-month horizon, FULL cleaned history')
res['h1_five_year'] = score(r1[r1['origin'] >= cut5], '1-month horizon, LAST FIVE YEARS')
res['h1_production'] = score(apply_breaks(r1, AE), '1-month horizon, production window set')
