"""ADNOC Distribution study — python-docx builder helpers, house style.
Palette: canvas 1C3A36 · panel EAF0EE/EFF3F1 · cream F6F1E6 · gold C0A45F · brass 896F36."""
import json, os
...
def box(lines, fill=F_CREAM):
    t = doc.add_table(rows=1, cols=1); _fixed(t)
...
def masthead():
    ...
    p = c.paragraphs[0]
    r = p.add_run('Testahil · Independent Valuation Study')
    r.bold = True; r.font.size = Pt(11); r.font.color.rgb = WHITE
    r2 = p.add_run('   Educational analysis · not investment advice')
