"""ADNOCDIST (ADNOC Distribution, ADX) study — Step 0.0 data-quality gate +
Step 0 calibration gate.
...
robust_verdict across bootstrap blocks {2,3,4}). ADNOCDIST is NEW coverage: it
has no committed fitted_configs entry to reconcile a verdict against, so the
...
assert 'ADNOCDIST' not in reg['panel_names'], "ADNOCDIST already covered — not new coverage"

# ---- Step 0.0 — data-quality gate ----------------------------------------
df_raw = raw_load(os.path.join(HERE, 'ADNOCDIST_Stock_Price_History.csv'))
df, dq_log = clean_ohlc(df_raw, 'ADNOCDIST', verbose=True, market='AE')
scr = screen(df)
