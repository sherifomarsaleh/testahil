...
DISCLOSED_PE = {2024: 100.0 * CP.v('util_pe_fy24'), 2025: 100.0 * CP.v('util_pe_fy25')}
DISCLOSED_PP = {2024: 100.0 * CP.v('util_pp_fy24'), 2025: 100.0 * CP.v('util_pp_fy25')}
...
# ============================================================================
# 3. THE SIGN TEST, AND WRITING THE TWO RECORDS
# ============================================================================
# MATERIALITY IS THE HOUSE'S OWN 5% BAR, reused rather than minted — the same line
# [R-ANCHOR-01] uses for a contested judgement, so this study does not carry a cutoff of
# its own. A judgement below it is RECORDED and not counted, because dropping it would let
# the count be tuned by moving the bar.
MATERIAL = [j for j in JUDGEMENTS if j['share_of_value'] >= 0.05]
N_UP = sum(1 for j in MATERIAL if j['value_adopted'] > j['value_alternative'])
N_DOWN = sum(1 for j in MATERIAL if j['value_adopted'] < j['value_alternative'])


def _sign_p(up, down):
    """Two-sided exact binomial against a fair coin. n = up + down; ties are excluded
    because a fork resolved the same way both ways is not a fork."""
    n = up + down
    if n == 0:
        return None
    k = max(up, down)
...
P_SIGN = _sign_p(N_UP, N_DOWN)
# FLAGGED, NEVER FAILED. A company can genuinely deserve a consistent read, and a gate that
# failed on one would push studies to resolve forks INCONSISTENTLY to stay green, which is
# the opposite of what this measures. What a study may not do is go unmeasured.
FLAGGED = bool(P_SIGN is not None and P_SIGN < 0.05)


def main():
    # the record must reproduce what the study publishes, or it is describing another model
    _pub = json.load(open(os.path.join(HERE, 'study_numbers.json'), encoding='utf-8'))
    for _k, _lens in ((FRAME_A, 'dcf_normalisation_own_beta'),
                      (FRAME_B, 'dcf_prolonged_own_beta')):
        assert abs(BASE[_k] - _pub['lenses'][_lens]) < 1e-9, (
            'this generator reads %.10f for %s where the study publishes %.10f — one of '
            'the two is describing a model the other does not have'
            % (BASE[_k], _lens, _pub['lenses'][_lens]))

    diag = dict(
        _='GENERATED by diagnostics_borouge.py from this study\'s own model. Never hand '
          'edited, and NEVER read back: a quantity solved from a price and returned to the '
          'valuation is the reverse-engineered rate the protocol prohibits outright.',
        ticker='BOROUGE', rule='R-ENF-05',
        published_central=float(BASE[FRAME_A]), published_spot=float(SPOT_AED),
        reverse_read=dict(
            quantity='utilisation of nameplate capacity, terminal year, per cent',
            why_this_quantity=(
                "it is this study's own crux — whether navigation through the Strait of "
                "Hormuz is restored during 2026 — expressed in the unit the company "
                "discloses every quarter, so a reader can check it against the filings "
                "rather than take it on trust"),
            solved_how=('bisection on a parallel shift across the five explicit years, '
                        'every other driver held at its published value, calling the '
                        "study's own shipped forecast function rather than a "
                        're-implementation'),
            implied_by_price={k: float(v) for k, v in IMPLIED_UTIL.items()},
            study_forecast={k: float(v) for k, v in STUDY_UTIL.items()},
            company_disclosed=dict(
                polyethylene={str(y): float(v) for y, v in DISCLOSED_PE.items()},
                polypropylene={str(y): float(v) for y, v in DISCLOSED_PP.items()},
                total_against_nameplate={str(y): float(v)
                                         for y, v in DISCLOSED_TOTAL.items()},
                h1_2026_annualised=float(H126_TOTAL)),
            flat_discount_rate={
                k: {kk: (float(vv) if isinstance(vv, (int, float)) else vv)
                    for kk, vv in FLAT[k].items()} for k in FLAT},
        ))
    json.dump(diag, open(os.path.join(HERE, 'diagnostics.json'), 'w', encoding='utf-8'),
              indent=1)

    cj = dict(
        _='GENERATED by diagnostics_borouge.py. Each fork is priced by RE-RUNNING the '
          "study's own model with one thing moved, never by estimating what it would do.",
        ticker='BOROUGE', rule='R-ENF-05',
        published_central=float(BASE[FRAME_A]), published_spot=float(SPOT_AED),
        published_central_frame_B=float(BASE[FRAME_B]),
        materiality='5% of value, the house bar, reused rather than minted here',
        judgements=JUDGEMENTS,
        n_total=len(JUDGEMENTS), n_material=len(MATERIAL),
        n_up=N_UP, n_down=N_DOWN, sign_test_p=P_SIGN, flagged=FLAGGED,
        flagged_means='FLAGGED IS NOT FAILED — a company can deserve a consistent read, '
                      'and failing on one would push studies to resolve forks '
                      'inconsistently to stay green. What a study may not do is go '
                      'unmeasured.')
    json.dump(cj, open(os.path.join(HERE, 'contested_judgements.json'), 'w',
                       encoding='utf-8'), indent=1)

    print('BOROUGE reverse read — the price is paying for terminal utilisation of')
    for k in (FRAME_A, FRAME_B):
        print('  %-14s %6.1f%% implied by AED %.4f, against this study\'s %.1f%%'
              % (k, IMPLIED_UTIL[k], SPOT_AED, STUDY_UTIL[k]))
    print('  the company disclosed %.1f%% and %.1f%% polyethylene in FY2024 and FY2025, '
          'and %.1f%% of nameplate across both lines in the annualised first half of 2026'
          % (DISCLOSED_PE[2024], DISCLOSED_PE[2025], H126_TOTAL))
    print('\n%d contested judgements, %d material at the 5%% bar, %d up / %d down'
          % (len(JUDGEMENTS), len(MATERIAL), N_UP, N_DOWN))
    print('  two-sided sign test p = %s%s'
          % ('n/a' if P_SIGN is None else '%.3f' % P_SIGN,
             '   FLAGGED — every fork one way' if FLAGGED else '   no lean'))
    for j in sorted(MATERIAL, key=lambda x: -x['share_of_value']):
        print('  %-6s %6.2f%%  %s'
              % ('UP' if j['value_adopted'] > j['value_alternative'] else 'DOWN',
                 100 * j['share_of_value'], j['name'][:78]))
    return 0
...
DIAG = {
    ...
    'also_solved_on_the_shared_instrument': {
        ...
        'reading':
            ...
            '%.3f%% on the normalisation branch — %.0f basis points — and %.3f%% against '
            'the same %.3f%% on the prolonged branch, which is %.0f basis points. The '
