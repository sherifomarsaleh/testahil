"""BOROUGE study - Step 0.0 data-quality gate + Step 0 calibration gate.

Production-identical: the live AE fit is read from engine/fitted_configs.json
(never quoted from memory); every production transform is applied
(data_quality.clean_ohlc -> backtest_v3 on the calendar 3-month horizon ->
apply_breaks -> scale-normalisation crps/spot -> robust_verdict across bootstrap
blocks {2,3,4}). BOROUGE is NEW coverage, so the reconciliation assert is on the
FIT ITSELF: the (nu, width_cal) used must be exactly the committed AE production
fit, and the market-panel verdict must match the committed registry.
"""
import sys, os, json
...
MKT_VERDICT = reg['market_verdict']

# ---- Step 0.0 - data-quality gate ----------------------------------------
df_raw = raw_load(os.path.join(HERE, 'BOROUGE_Stock_Price_History.csv'))
df, dq_log = clean_ohlc(df_raw, 'BOROUGE', verbose=True, market='AE')
scr = screen(df)
...
summ = dict(
    ...
    market_gate=dict(verdict=MKT_VERDICT, skill=reg['market_skill'],
    ...
    span_years=float(span_yr), max_abs_log=float(scr['max_abs_log']),
    flat_frac=float(scr['flat_frac']), jump_threshold=float(jump_threshold('AE')),
)
...
print(f"\nStep 0: {len(r)} post-break windows ({n_all - len(r)} pre-break dropped) | "
      f"skill(norm) {skill:+.4f} | verdict {verd} | cov50/80/90 "
      f"{r['in50'].mean():.2f}/{r['in80'].mean():.2f}/{r['in90'].mean():.2f} | "
      f"PIT mean {pit.mean():.3f}")
print("  CI by block:", {b: detail[b] for b in (2, 3, 4)})
print("  PIT hist (10 bins):", pit_hist)
