"""BOROUGE beta — tier-1 own-stock weekly regression against an equal-weight ADX-DFM composite
built from the full engine/raw_ohlc/AE library (house pattern: CLHO / RMDA / SWDY / SCEM / ARCC),
...
out = dict(
    ...
    plausibility=dict(
        ...
        net_cash_note=("The company carries roughly 0.8x net debt to EBITDA and its earnings are "
                       "levered to a global commodity price benchmark, both of which argue for "
                       "an estimate at or above the cyclical band rather than below it.")),
)
