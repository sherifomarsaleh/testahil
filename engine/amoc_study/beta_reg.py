"""AMOC beta — tier-1 own-stock weekly regression vs an equal-weight EGX
...
amoc, _ = clean_ohlc(load_ohlc(os.path.join(HERE, 'AMOC_Stock_Price_History.csv')),
                     'AMOC', verbose=False, market='EG')
amoc = amoc.set_index('Date')['Price']
...
cut = amoc.index.max() - pd.DateOffset(years=5)
wk_amoc = weekly(amoc[amoc.index >= cut])
rets = {}
...
re = np.log(wk_amoc / wk_amoc.shift(1)).dropna()
al = pd.concat([re.rename('amoc'), mkt.rename('mkt')], axis=1).dropna()
x, y = al['mkt'].values, al['amoc'].values
n = len(x)
