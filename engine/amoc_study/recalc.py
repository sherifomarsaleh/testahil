...
checks = [
    ...
    ('Fundamental — weighted central', g('Fundamental Valuation', f"C{FV['central']}"),
     D['central'], 0.02),
    ('Product lines — calendar 2025 revenue', g('Product Lines', f"E{LG_['rev_cy25']}"),
     BASE['rev_cy25'], 1.0),
    ('Cost build — SOLVED feedstock differential vs crude parity',
     g('Product Lines', f"B{LG_['feed_diff']}"), D['unit']['feed_diff'], 0.0002),
    ('Cost build — base-oil crack multiple, solved from the disclosed table',
     g('Product Lines', f"B{LG_['crack']['oil']}"), D['unit']['crack']['oil'], 0.0005),
    ('Cost build — fuel-slate crack multiple (near crude parity)',
     g('Product Lines', f"B{LG_['crack']['fuel']}"), D['unit']['crack']['fuel'], 0.0005),
    ('Cost build — 2026E cost of sales', g('Product Lines', f"{UC_[0]}{LG_['cogs']}"),
     F['rev'][0] - F['gp'][0], 1.0),
    ('Cost build — 2026E base-oil margin is an OUTPUT',
     g('Product Lines', f"{UC_[0]}{LG_['m_spec']}"), D['unit']['line_margin']['oil'][0], 0.0002),
    ('Cost build — 2026E fuel-slate margin is an OUTPUT (and is negative)',
     g('Product Lines', f"{UC_[0]}{LG_['m_fuel']}"), D['unit']['line_margin']['fuel'][0], 0.0002),
    ('Product lines — 2030E blended gross margin is an OUTPUT',
     g('Product Lines', f"{UC_[4]}{LG_['gm']}"), F['gm'][4], 0.0002),
    ('Product lines — calendar 2025 profit after tax', g('Product Lines', f"B{LG_['pat_cy25']}"),
     BASE['pat_cy25'], 1.0),
    ('Product lines — 2030E total revenue', g('Product Lines', f"{UC_[4]}{LG_['rev']}"),
     F['rev'][4], 1.0),
