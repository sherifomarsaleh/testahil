...
class Book:
    ...
    def evaluate(self, expr, sheet):
        ...
        while True:
            ...
            e = e[:m.start()] + repr(float(v or 0)) + e[m.end():]
        # '^' is Excel's exponentiation operator and is translated to Python's '**'.
        # Precedence matches for every expression this workbook uses; the one place the two
        # languages differ is a unary minus immediately left of the operator (Excel reads
        # -2^2 as +4, Python as -4), which is rejected below rather than silently mis-read.
        if not re.fullmatch(r'[-+*/().^ 0-9eE]+', e):
            raise ValueError(f'unparsed formula fragment: {expr!r} -> {e!r}')
        if re.search(r'(^|[(*/+])\s*-[\d.]+\s*\^', e):
            raise ValueError(f'ambiguous unary minus before ^ in {expr!r}')
