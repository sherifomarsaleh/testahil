"""ELEC beta — tier-1 own-stock weekly regression vs an equal-weight EGX
...
elec, _ = clean_ohlc(load_ohlc(os.path.join(HERE, 'ELEC_Stock_Price_History.csv')),
                     'ELEC', verbose=False, market='EG')
elec = elec.set_index('Date')['Price']
...
cut = elec.index.max() - pd.DateOffset(years=5)
wk_elec = weekly(elec[elec.index >= cut])
rets = {}
...
re = np.log(wk_elec / wk_elec.shift(1)).dropna()
al = pd.concat([re.rename('elec'), mkt.rename('mkt')], axis=1).dropna()
x, y = al['mkt'].values, al['elec'].values
n = len(x)
