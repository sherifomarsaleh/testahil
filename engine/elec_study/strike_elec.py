"""ELEC — forward cone strike via the identical production chain
...
Q_ANNUAL = 0.0   # ELEC pays no dividend (confirm vs research dossier)

prof = MP.PROFILES['EG']
raw = load_ohlc(os.path.join(HERE, 'ELEC_Stock_Price_History.csv'))
df, rep = clean_ohlc(raw, 'ELEC', verbose=False, market='EG')
