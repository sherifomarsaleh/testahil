...
6. Beta. From the stock's own price history regressed vs its own local index (EGX30 for EGX names) — the SAME beta produced by the beta procedure and the wacc_builder usability gate, not a second method.
   [ADDED 10-Aug-2026, per the AMR third edition] THE REGRESSOR IS A REAL, PUBLISHED INDEX OF THE STOCK'S OWN MARKET, AND NOTHING ELSE MAY STAND IN FOR ONE. Three substitutes are specifically prohibited as the regressor, each having been used and then overturned on this very name. (i) A CONSTRUCTED COMPOSITE of the names this project happens to cover is not a market: it is a selection-biased subset whose own sector mix becomes the "market", and the subject is one of its constituents, so the stock is regressed partly against itself. It is a DIAGNOSTIC, never the regressor. (ii) ANOTHER COUNTRY'S INDEX, even for a genuinely dual-listed share, prices the company against a different market's cycle. (iii) AN INDEX FUND TRADING IN A DIFFERENT SESSION is a real index but a mistimed one. Each substitute gave a MATERIALLY WRONG ANSWER, in both directions and by a wide margin: on Americana the Saudi-line-vs-Tadawul regression gave 0.894, a covered-UAE composite 0.586, a US-listed MSCI UAE fund 0.469, and the company's own exchange index — the FTSE ADX General — 1.026. The composite and the fund understated beta by roughly half and would have RAISED the published value by about a quarter; the Saudi line understated it too. THE CORRECT ORDER WHEN THE INDEX IS NOT TO HAND: attempt it, log the attempt and its outcome in the Sweep Register, and ASK THE USER FOR THE SERIES — exactly as the primary-source rule already requires for company filings, and it worked here, the user supplying 3,884 daily closes on request. Publish on a substitute only if it is refused, and then flag it as the study's largest open item and re-issue when the index arrives. SCREEN THE INDEX like any other series before use (duplicate dates, non-positive levels, trading-day density, largest single-session move, weekday pattern) and commit it under engine/market_indices/{MARKET}_{INDEX}.csv — NEVER under engine/raw_ohlc/{MARKET}/, which is the calibration panel of covered STOCKS and would silently admit an index into a panel it has no business in. VALIDATE THE INDEX ON ITS OWN CONSTITUENTS and publish the result: regress two or three of its heavyweights the same way and state where they land, because an index that does not price its own large constituents sensibly is not measuring the market it names. APPLY A DIMSON LEAD-LAG CORRECTION ONLY when stock and index genuinely close at different moments, and say so either way — on same-exchange, same-session data the sum should land near the plain coefficient, and reporting it is how the claim "no correction needed" becomes evidence rather than assertion. STATE THE PRECISION: publish the standard error and the one-error band, and name the alternatives that band cannot be distinguished from.
