...
6. Beta. From the stock's own price history regressed vs its own local index (EGX30 for EGX names) — the SAME beta produced by the beta procedure and the wacc_builder usability gate, not a second method.
   [ADDED 10-Aug-2026, per the AMR third edition] THREE PROCEDURAL RULES TRAVEL WITH THE REGRESSOR, and two further substitutes are prohibited alongside the constituent composite.
   PROHIBITED, each used on AMR before the index arrived and each wrong: (a) ANOTHER COUNTRY'S INDEX, even for a genuinely dual-listed share — AMR's Riyadh line on TASI gave 0.894, pricing the company against a different market's cycle; the dual-listing trap is about matching the SERIES to the EXCHANGE, never a licence to swap the index. (b) AN INDEX FUND TRADING IN A DIFFERENT SESSION — a US-listed UAE fund gave 0.469, half the right figure, because it prices hours after the ADX close. Roll-call on that one name: 0.894 other-country index, 0.586 covered-name composite, 0.469 differently-timed fund, 0.930 FADGI and conforming. The three non-conforming estimates understated beta by up to half and would have raised published value by about a quarter — in the OPPOSITE direction to the FERTIGLB precedent, which is the point: a bad regressor is not conservative, it is just wrong, and it can err either way.
   (i) DIMSON ON BY DEFAULT. own_stock_beta() applies the lead-lag correction and should: it costs nothing when stock and index close together — the sum lands near the plain coefficient, which is how the claim 'no correction needed' becomes evidence instead of assertion — and it is the only thing that rescues a regressor whose session differs.
   (ii) VALIDATE THE INDEX ON ITS OWN CONSTITUENTS, AND PUBLISH THE RESULT. Regress two or three of its heavyweights the same way and state where they land. On FADGI: Aldar 1.58, Emaar 1.50, ADCB 1.36, Agthia 0.42 — property and banking above the index, a consumer staple well below, which is what an index concentrated in property, banking and the Abu Dhabi holding companies should produce, and which puts a restaurant operator at the market average by evidence rather than by accident. An index that cannot price its own heavyweights sensibly is not measuring the market it names.
   (iii) STATE THE PRECISION, ALWAYS: SE, the 90% interval, and the alternatives that interval cannot exclude. AMR's conforming beta is 0.930 with SE 0.412 and a CI90 of 0.25 to 1.61; the resolver flags it weak and the study says in plain language that nothing rests on beta being exactly right. A beta quoted bare is a false precision on the input the valuation is most sensitive to.
   RE-DERIVING MEANS DELETING THE LOCAL SCRIPT, not running it alongside the sanctioned call: AMR's beta_reg.py is deleted and replaced by a single own_stock_beta() call gated by assert_beta_provenance(), with the provenance record carried in study_numbers.json under wacc.beta_provenance. Leaving a hand-rolled script beside the sanctioned one is how the next edition silently regresses on a composite again. MODON is NOT yet conforming — its published WACC still rests on an 18-name equal-weight UAE composite.
