...
ENFORCEMENT, STATED PLAINLY RATHER THAN DRESSED UP [R-ENF-01]. engine/method_reassessment/criterion3.py carries the phase map and PRINTS THE CLAUSE SPLIT at every run, so which clauses gate and which are reported is read off the instrument rather than remembered; the audit itself is a committed artefact at engine/valuation_calibration/EXPENSIVE_CALLS_AUDIT_08-09-2026.md, naming every expensive call and the measurement that establishes its cause. THAT IS THE WHOLE OF IT AND NO MORE IS CLAIMED FOR IT: no CI job runs this clause, so it is not a gate in the sense every other enforcement rule here uses the word, and A RULE ADOPTED WITH A GATE THAT DOES NOT EXIST IS WORSE THAN ONE THAT ADMITS IT — it reads as enforced and binds on nothing. What does exist beside the criterion is scripts/check_clause_g_negative_control.py, which fires the clause on its own conditions including the clean case the one-sidedness turns on, a cell far ABOVE its price that must NOT fire; wiring either into the build is its own step and is not claimed here.
