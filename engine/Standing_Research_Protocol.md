...
### [R-LESSON-01] Every lesson is registered, explained plainly, and scoped

ADOPTED 31-Aug-2026, per instruction — "I want lessons learnt from all the walk
forward fundamental training from all stocks to be kept in a register, explained
simply and then categorized if they are to be learnt and applied to the stock
only, to the stock class (for example real estate developers) or for all stocks."

A LESSON IS USELESS UNTIL YOU KNOW HOW FAR IT CARRIES. This project had been
accumulating findings in three different places with no scope on any of them: the
standing protocol (which implicitly means "every study, always"), each study's own
critique response and self-audit (which implicitly means "this study, once"), and
the Fundamental Driver Ledger (which was meant to mean "the next company of this
class" and held a single entry). Nothing said which findings travelled and which
did not, so in practice the universal ones were applied and the rest were lost.

THE REGISTER IS `engine/Lessons_Register.md`, GENERATED FROM
`engine/lessons_register.py` BY `engine/build_lessons_register.py` AND NEVER
HAND-EDITED. A document that states a fact which moves must not be the thing that
remembers it — the same rule the as-of stamps and the band records already obey.

EVERY LESSON CARRIES A SCOPE, AND THE THREE ARE NOT INTERCHANGEABLE:

  ALL    about method, arithmetic, or how work is checked. Binds on every study
         ever run. These are the expensive ones and are usually learned by
         getting something wrong.
  CLASS  true of every company that works the same way — every off-plan
         developer, every telecom operator, every bank. The next study of that
         class must read them; a study of a different class must not, because a
         rule true of developers is not evidence about airlines.
  STOCK  true of one company and nothing else — a quirk of its disclosure, its
         history, its accounting. APPLYING A STOCK LESSON TO ANOTHER COMPANY IS
         SUPERSTITION.

CHOOSING THE SCOPE IS THE JUDGEMENT AND IT IS COSTLY IN BOTH DIRECTIONS. Too
narrow and the next study repeats the mistake; too broad and one company's quirk
becomes a house rule nobody can dislodge. WHEN GENUINELY UNSURE, FILE AT THE
NARROWER SCOPE AND WIDEN WHEN A SECOND COMPANY SHOWS THE SAME THING — one
observation is not a pattern.

EVERY LESSON ALSO RECORDS HOW IT WAS LEARNED, because the evidence differs
enormously in strength: `walk_forward` (measured against out-of-sample outcomes
across many origins — the strongest evidence this project produces), `critique`
(found by an external reviewer of a delivered study), `self_audit` (found by
re-examining our own build before anyone else read it), `build` (found while
building or rendering, usually because a check reported clean on something it was
not looking at). WHERE TWO LESSONS DISAGREE, THE WALK-FORWARD ONE WINS, and the
register says which is which rather than presenting them as equals.
...
tested. The field is required and may not be empty.

WHEN TO APPEND. A walk-forward run, a critique response, a self-audit or a QC
gate that produced a finding worth keeping appends it in the same commit that
records the finding. `lessons_register.lessons_for(ticker, klass)` is what a new
study reads before it starts: every ALL lesson, plus that class's, plus that
name's.

ENFORCED FROM OUTSIDE per [R-ENF-01], because a register that validates itself is
a self-attested boolean. `scripts/check_lessons_register.py` FAILS (never warns)
if the document is not its generator's byte-for-byte output, if any id fails to
resolve in both directions, if a registered class carries no lesson, if a lesson
is scoped to a company this repository does not study, or if a walk-forward run
exists on disk with no lesson behind it. THE POPULATION IS ANCHORED OFF THE
WALK-FORWARD DIRECTORIES ON DISK per [R-ENF-04] — an empty register FAILS rather
than reporting clean, because an empty result is not a clean result.
`scripts/check_lessons_register_negative_control.py` injects each of those
defects and fails if any is missed; both run in CI.

THE REGISTER STATES ITS OWN GAP RATHER THAN HIDING IT. As at adoption only one
company had been through a full walk-forward run, so every walk-forward lesson
came from that one name and the rest were learned from delivered studies,
