"""ARCC — forward cone strike via the identical production chain
...
Q_ANNUAL = 5.34 / 59.00   # Declared FY2025 dividend of EGP 5.34 per share over the
                          # 06-Aug-2026 close of EGP 59.00 = 9.05%. The company distributes
                          # about 56% of profit, so the carry anchor is ln(1+rf) - ln(1+q)
                          # rather than ln(1+rf) alone; ignoring it would put a 9-point
                          # upward drift into a PRICE forecast that the shares do not earn.

prof = MP.PROFILES['EG']
raw = load_ohlc(os.path.join(HERE, 'ARCC_Stock_Price_History.csv'))
df, rep = clean_ohlc(raw, 'ARCC', verbose=False, market='EG')
