"""ARCC study — Step 0.0 data-quality gate + Step 0 calibration gate.
...
robust_verdict across bootstrap blocks {2,3,4}). ARCC is NEW coverage: it has
...
# `assert 'ARCC' not in reg['panel_names'], "ARCC already covered -- not new coverage"`,
...
# the live production one. Now that ARCC sits in the panel the reconciliation is STRONGER
# than it was, because the name is inside the very panel (nu, width_cal) were fitted on.
_IN_PANEL = 'ARCC' in reg['panel_names']
print("Step 0 reconciliation: ARCC is %s the committed EG panel of %d names; the fit used "
...
df_raw = raw_load(os.path.join(HERE, 'ARCC_Stock_Price_History.csv'))
df, dq_log = clean_ohlc(df_raw, 'ARCC', verbose=True, market='EG')
scr = screen(df)
