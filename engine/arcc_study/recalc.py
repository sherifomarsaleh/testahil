...
XLSX = os.path.join(HERE, 'ARCC_Valuation_Model_06082026_public.xlsx')
...
checks = [
    ...
    ('DCF equity value', ('DCF', 'B38'), DCF['equity'], 1.0),
    ('DCF fair value per share', ('DCF', 'B40'), DCF['fv'], 0.02),
    ('DCF terminal return on invested capital', ('DCF', 'B24'), DCF['roic_term'], 0.0005),
    ('DCF reinvestment rate', ('DCF', 'B25'), DCF['rr_term'], 0.0005),
    ('Cost of equity — explicit', ('DCF', 'C39'), W['ke_exp'], 0.0002),
    ('WACC — explicit window', ('DCF', 'C40'), W['wacc_exp'], 0.0002),
    ('WACC — terminal', ('DCF', 'C46'), W['wacc_term'], 0.0002),
    ('Effective tax rate built in the sheet', ('DCF', 'C47'), H['tax_eff'], 0.0002),
    ('Terminal beta re-levered', ('DCF', 'C44'), W['beta_term'], 0.001),
    ('Market capitalisation', ('DCF', 'C42'), M['mktcap'], 1.0),
    ('Cash at the valuation date', ('DCF', 'B34'), DCF['cash_at_val'], 1.0),
    ('NCI deducted in the bridge', ('DCF', 'B37'), -D['inputs']['nci']['value'], 0.5),
    ('Bridge enterprise value', ('EV Bridge', 'B7'), DCF['ev'], 1.0),
    ('Bridge equity value', ('EV Bridge', 'B10'), DCF['equity'], 1.0),
    ('Bridge terminal value share', ('EV Bridge', 'B12'), DCF['tv_share'], 0.001),
    ('Bottom-up FY2025 revenue', ('Unit Build', 'B19'), D['bottom_up'][0]['rev'], 1.0),
    ('Bottom-up FY2025 EBITDA (an OUTPUT)', ('Unit Build', 'B36'),
     D['bottom_up'][0]['ebitda'], 1.0),
    ('Bottom-up FY2025 realised price', ('Unit Build', 'B20'),
     D['bottom_up'][0]['price'], 1.0),
    ('Depreciation triangulation — adopted average', ('Unit Build', 'B48'),
     D['dna_triangulation']['adopted'], 0.5),
    ('Validation: bottom-up revenue vs disclosed', ('Unit Build', 'B56'),
     D['bottom_up'][0]['rev'] / D['inputs']['rev_fy25']['value'] - 1, 0.001),
    ('Validation: operating profit vs DISCLOSED', ('Unit Build', 'B59'),
     (D['bottom_up'][0]['ebitda'] - D['dna_triangulation']['adopted'])
     / D['inputs']['ebit_fy25']['value'] - 1, 0.001),
    ...
    ('Income statement FY2030E profit after tax', ('Income Statement', 'I13'), F['pat'][4], 1.0),
    ('Balance sheet FY2025 equity (disclosed)', ('Balance Sheet', 'D12'),
     D['inputs']['eq_fy25_rep']['value'], 0.05),
    ('Balance sheet FY2030E equity', ('Balance Sheet', 'I12'), F['equity'][4], 1.0),
    ('Balance sheet liabilities derived', ('Balance Sheet', 'B20'),
     D['equity_gap']['derived_liabilities'], 0.05),
    ('Cash flow FY2026E free cash flow to the firm', ('Cash Flow', 'B14'), F['fcff'][0], 1.0),
    ('Relative lens implied value', ('Relative & Normalized', 'B21'),
     LN['values']['Relative multiples'], 0.02),
    ('Normalised lens implied value', ('Relative & Normalized', 'B29'),
    ...
    ('Summary — terminal value share beside the DCF lens', ('Summary', 'F5'),
     DCF['tv_share'], 0.001),
    ('Summary — market capitalisation', ('Summary', 'B13'), M['mktcap'], 1.0),
