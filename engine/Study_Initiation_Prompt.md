...
- **Reference study to match** — the MODEL REPORT is **ADNOCLS_Valuation_Study_09-08-2026** (`engine/adnocls_study/`), minus the section "What changed in these editions, and why"; the built document is `engine/model_report/MODEL_REPORT_09-08-2026.docx` and the canonical prose is `engine/model_report/MODEL_REPORT_19-08-2026.md`. Its sections list, sheet list, **per-section content** and research depth are the standard for EVERY class. The class picks the LENS PATTERN only, and **the reference set is closed at exactly three names** — ADNOCLS (operating company / diversified industrial), ADCB (bank), ALPHADHABI (holding company). No other company is a template or a reference study; if the class is none of these three, adapt the nearest pattern's lens inside ADNOCLS's skeleton and say which you adapted and why. Do not go looking for a fourth exemplar. **The skeleton is not the standard**: the per-section content contract is `engine/model_report/model_report_spec.py`, and a section heading with no arithmetic under it is a lens asserted, not built.
- **Sector** — for gate item (c) below.

Then follow the TESTAHIL Standing Research Protocol end-to-end without asking: read live state first (market_profiles.py + fitted_configs.json from the repo), then Step 0.0 data-quality gate → Step 0 calibration gate (scale-normalized, carry-anchored) → Step 2A four-ring Information Sweep → financials → build the 16-section Word + 16-sheet Excel + standalone bibliography document matching the MODEL REPORT, ADNOCLS_Valuation_Study_09-08-2026 (`engine/adnocls_study/`; machine-readable skeleton and depth bar in `engine/research_protocol.py` `MODEL_STUDY` / `MODEL_STUDY_DEPTH`, per-section content contract in `engine/model_report/model_report_spec.py`) → run `python3 engine/model_report/check_model_report.py --study … --xlsx … --biblio …` on the DELIVERED files → unprompted QC gate as a filled evidence table, with that gate's per-section output pasted in as the evidence for item (a) and `ModelStudyChecklist` attested alongside SIGCM.
...
(a) Study structure, content, format AND research depth exactly match the MODEL REPORT (ADNOCLS_Valuation_Study_09-08-2026, `engine/adnocls_study/`) — including the standalone bibliography document and every depth-bar standard in `MODEL_STUDY_DEPTH`. **Evidence, not attestation**: paste the per-section output of `engine/model_report/check_model_report.py` here, with zero FAILs. `assert_model_study()` refuses a checklist attested without those findings attached.
