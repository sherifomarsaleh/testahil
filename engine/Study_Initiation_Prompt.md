...
(s3) **[R-REAL-01] Every per-unit money path says what it does in REAL terms, and `python3 scripts/check_real_terms_paths.py` is clean** — write the path through `engine/real_terms.py`'s `nominal_from_real()`, which takes a REAL rate and lets the house inflation ladder supply the nominal, so a price *"tracking inflation"* at 5% against a 16% ladder has to be written as **−9.5% real**, which nobody writes by accident. Commit a `real_terms_block` naming each price-class path with its kind, its measured `real_change`, a mechanism from the closed list (`administered_price`, `traded_commodity_flat_in_hard_currency`, `contractual_escalator`, `regulated_tariff_glide`, `competitive_erosion_measured_in_own_history`) and the disclosure establishing it from the filings; an empty disclosure switches the check off rather than declaring it. The basis follows the currency the price is SET in — a local price against the domestic ladder, a hard-currency price against long-run foreign inflation. **The rule does not say a real decline is wrong** — an administered price genuinely lags inflation — it says the claim may not be unsaid, because a revenue line that does not move with inflation while the cost lines do manufactures a margin collapse and the model then reports the collapse as a finding.
