...
- **Financial statements — primary sources only, and this one you DO ask me about.** Build the historical income statement, balance sheet and cash flow **only** from the company's own issued statements and full disclosures, read from an official home: its website / investor-relations page, the exchange's disclosure portal, or the regulator's filing archive. Aggregators, broker notes, press coverage and search-result extracts are cross-checks, never build sources. **If you cannot reach those statements — the site is blocked or down, the filings are not published, the documents will not parse, or fewer than TWO COMPLETE FINANCIAL YEARS can be assembled — STOP WORK AND ASK ME WHAT TO DO.** Two complete years is the floor: full income statement, balance sheet, cash flow and notes for each, from official sources, for two consecutive audited financial years. Three is what gate item (e) wants; below three but at or above two, **build and say so** — state on delivery which year is missing, where you looked for it and what it costs the forecast, and record the shortfall in the QC evidence table rather than passing item (e) silently. Below two, you stop. Do not reconstruct the statements, do not substitute unofficial data, and do not deliver a model on a "best available data, labelled as such" caveat. Try properly before you stop: work all three official homes, and treat a 403/407 or TLS failure as an egress-proxy fault until you have checked it, not as a company-website failure. When you stop, tell me the ticker, what you needed, every official source you tried with its URL and how it failed, what that blocks downstream, and the options as you see them — then wait for my answer. No partial model, no provisional build, nothing delivered until I have answered.
...
(s) **Primary-source access.** Every historical figure traces to the company's own issued statements,
read from the company website/IR, the exchange disclosure portal or the regulator's archive.
Evidence: the official sources actually read, listed per statement and per period, and the count of
complete financial years obtained — **two is the floor**, three is what item (e) wants, and anything
between the two is stated openly here and against item (e) rather than passed over. An aggregator or
a search-result extract anywhere in the *build* path is a hard fail — cross-check use is fine and
...
**Two things can still block the run, and they stop it in different ways.**

*The price history* is a hard stop. Market data is not reachable from the build environment, so the
OHLC has to be in `engine/raw_ohlc/{MARKET}/{TICKER}.csv` or attached to the message. The prompt
instructs a hard stop rather than a reconstructed series, because a fabricated price history would
silently corrupt the calibration gate, the beta regression and the Monte Carlo cone at once.

*The company's financial statements* are a **stop-and-ask**. Where the price series has one canonical
location, the filings have several official homes — the company's IR page, the exchange portal, the
regulator's archive — so unreachability is a judgement about all of them together, and the remedy is
usually something the requester holds (the PDFs) or can authorise. That makes it a question, not a
refusal. It is nonetheless a genuine halt: no partial model, no provisional build, nothing delivered
until the answer comes back. The precedent is ELEC (05-Aug-2026), where the proxy blocked the company
website and every portal, the study was built from web-search extracts on a disclosed caveat, and the
exact statement line items were never obtained. A caveat is not a source.
