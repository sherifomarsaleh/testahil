...
(s1) **[R-ASSET-01] If the class carries an asset-based lens, the `asset_base_record` is committed and `python3 scripts/check_asset_base.py` is clean** — the quantity the value rests on (land bank, fleet, kilns, keys, installed capacity), its unit, its value, the date it is AS AT and the disclosure it came from. The test is an ORDERING, not an age: the asset base must be as at, or later than, the end of the information set the study claims to have read. Where the company has not restated it since, declare `not_restated_since` NAMING the later disclosures you checked and giving a reason — an empty reason switches the check off rather than declaring it. The scope is derived from `LENS_REGISTRY` by import, so a class string that resolves to nothing is RED rather than silently exempt.

(s2) **[R-COC-02] The cost-of-capital record declares the construction its terminal cost of equity reproduces under, and `python3 scripts/check_ke_reproduction.py` is clean** — `same_beta` or `relevered`, from that closed list, and a relevered terminal STATES the tax rate it was relevered at. Build the schedule through `engine/cost_of_capital.py`, which emits the field. The tolerance is float noise because this is an identity: a record storing rounded figures fails as a rounded record rather than being waved through. The gate solves the implied relevering rate and prints it in its own refusal so the fix is one line — but a rate solved out of the answer it explains is never accepted as the declaration.

(s3) **[R-COC-03] Country risk is charged ONCE and is never multiplied by beta.** Build the cost of equity through `engine/cost_of_capital.py`: `Ke = rf* + beta x ERP_mature + lambda x CRP`, both halves DERIVED from the registered total by `split_erp()` so no new number enters and the two cannot drift from the total they came from, and the components PUBLISHED rather than a single figure a reader cannot take apart. A published country premium is a TOTAL, so `beta x ERP_total` charges the country component at beta too — a claim about sovereign exposure that nobody made and that fell out of the algebra. Lambda is WHERE THE OPERATIONS ARE: default 1.0, taken from an audited geographic disclosure and never from company commentary, and a lambda below 1.0 REQUIRES a stated `crp_foreign` for where the rest of the money is earned — the module refuses the relief without the offset. A terminal struck this way declares `split_premium` and records the split it used; the retired construction reproduces only for records struck before 10-09-2026, because rewriting them would be rewriting history.

(s5) **[R-MACRO-02] The terminal real growth sits below the economy's long-run real growth, and the Fisher identity is exact in both directions.** Read `terminal_growth()` off the market's own path in `engine/macro_path.py` rather than typing a nominal rate. It REFUSES a real rate at or above `real_gdp_lt` — a company compounding faster than its economy forever becomes the economy — and the gap between the stated rate and the cap is the share of the economy the company is assumed to cede, which the study states rather than implies. Anywhere the study inverts the identity it uses `(1+g)/(1+pi)-1` exactly: two converters that are not exact inverses disagree BY CONSTRUCTION and agree only while real growth is zero, which is why nothing had ever seen it.

(s6) **[R-DCF-01] The valuation is assembled on ONE PAGE, and [R-SENS-01] the sensitivity grid is centred on the adopted case.** Build both through `engine/dcf_table.py`. `dcf_table()` reads the study's own committed numbers, owns no arithmetic, and RAISES where the present values do not reproduce the published enterprise value and value per share — if the table and the study disagree, the study is right and the table is broken. `sensitivity_grid()` refuses an even axis and refuses a centre cell that is not the central; a list axis (beta, FX) is SORTED with the adopted value marked in its place, never inserted at the middle position. ONE valuation function only: a second helper that re-implements the terminal is a second model hiding inside the block whose job is to test the first, and when a fix goes into one of two functions that do the same job the other one is now a defect. And anything the document will PRINT is committed to the numbers file at the moment it is computed — a line that is computed and not published is a line the document cannot print.
...
(s4) **[R-GAP-04] The review does not get to conclude that the gap is genuine, that the gap stands, or that it should be referred to me, until it has hunted OUR OWN ERROR exhaustively and come back empty — and it carries that hunt.** Referral is a last resort that has to be earned and evidenced, not a default. Record for each candidate what was examined, WHAT IT IS WORTH IN CURRENCY PER SHARE, and what would falsify it; a review that reaches "we do not know which" without that record is unstarted, not finished. At minimum, each priced: the study's own arithmetic worked BACKWARDS from the price through `engine/reverse_read.py` [R-ENF-05]; every typed constant against the model's own derived equivalent; every source string against what the filings actually disclose (a study saying a disclosure does not exist while the issuer publishes it has a hole in its SWEEP, reported whatever it is worth); standalone against consolidated on every ratio; each lens against the model's own forecast of the quantity that lens consumes; and the contested register tested for a lean INSIDE a category, not only across the file. **None of this weakens the absolute prohibition on moving a fair value toward the price.** Closing the gap is the OUTCOME of finding a real defect and never the aim — a defect is corrected identically whether it moves the answer toward the market or away from it, and finishing further from the price than you started is full compliance.

(s7) **[R-STAR-01] A central BELOW the traded price carries an air-tight WRITTEN case; a central above it does not.** Clear `engine/northern_star.py` — `from_numbers(TICKER)` returns the rung this study has to clear and what is missing from it, and `enforce()` refuses to issue. The burden rises with the gap, and the hunt of [R-GAP-04] is where the case's material comes from or is the reason there is no case. A central above the price needs no case at all: value not yet realised is the ordinary situation. **The fix is NEVER to move the number toward the price** — find the defect, or say in the study, in as many words, that the case is not made.

(s8) **[R-NEWS-01] The sweep looks at what the company says it will DO, and pins every item to the listed issuer.** The Company ring's announced-projects category is mandatory in `engine/research_sweep.py` and closes on a finding or a dated negative search — the requirement is that somebody LOOKED. Every forward-looking item names the LEGAL ENTITY that signed it and whether that entity is the listed issuer; unestablished is not the same as yes [R-ENF-04], and an item that is not the issuer's drives nothing and is carried as colour. It never touches historicals: SIGCM clause 1 stands absolutely, and news informs FORWARD DRIVERS only.
