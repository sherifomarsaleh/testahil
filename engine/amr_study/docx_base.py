...
def _rebalance(rows, widths, size):
    """Trim columns that are wider than their content needs and give the slack to the
    column that has the most text to carry. Column widths are still authored by hand —
    this only removes dead space, keeps the table's total width unchanged, and never
    narrows a column below the longest unbreakable word in it."""
    cpi = 135.0 / max(size, 1.0)          # characters per inch at this point size
    MIN_W = 0.50
    need, word = [], []
    for j in range(len(widths)):
        ...
        for r in rows:
            if j < len(r):
                cell = '' if r[j] is None else str(r[j])
                for line in cell.split('\n'):
                    longest_line = max(longest_line, len(line))
                for w_ in cell.split():
                    longest_word = max(longest_word, len(w_))
        need.append(longest_line / cpi)
        word.append(longest_word / cpi)
    out = list(widths)
    freed = 0.0
    for j, w in enumerate(widths):
        if w <= 1.10:
            continue
        target = max(need[j] / 0.55, word[j] / 0.95, MIN_W)
        if target < w:
            freed += w - target
            out[j] = target
    # Redistribute the freed width in proportion to how much text each column carries,
    # but never past the point where a column would itself become bloated. Anything that
    # cannot be placed is simply not used: a table narrower than the text block reads
    # better than one padded out to fill it.
    for _ in range(3):
        if freed <= 0.01:
            break
        room = [max(0.0, need[j] / 0.55 - out[j]) for j in range(len(widths))]
        total_room = sum(room)
        if total_room <= 0.01:
            break
        take = min(freed, total_room)
        for j in range(len(widths)):
            out[j] += take * room[j] / total_room
        freed -= take
    return out
...
def table(rows, widths, header=True, first_col_bold=False, size=9.3, header_fill=F_PANEL,
    ...
    widths = _rebalance(rows, widths, size)
