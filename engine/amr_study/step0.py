"""AMR study — Step 0.0 data-quality gate + Step 0 calibration gate.

Production-identical per the engine-reconciliation rule: the live AE fit is read
...
robust_verdict across bootstrap blocks {2,3,4}). AMR is NEW coverage: it has no
committed fitted_configs entry to reconcile a verdict against, so the
reconciliation ASSERT here is on the FIT ITSELF — the (nu, width_cal) used must
be exactly the committed AE production fit, and the market-panel gate verdict
must match the committed registry.
"""
import sys, os, json
...
assert 'AMR' not in reg['panel_names'], "AMR already covered — not new coverage"

# ---- Step 0.0 — data-quality gate ----------------------------------------
df_raw = raw_load(os.path.join(HERE, 'AMR_Stock_Price_History.csv'))
df, dq_log = clean_ohlc(df_raw, 'AMR', verbose=True, market='AE')
scr = screen(df)
...
# Uniformity of PIT: KS statistic against U(0,1)
ps = np.sort(pit)
n = len(ps)
ks = float(np.max(np.abs(np.arange(1, n + 1) / n - ps)).item()) if n else float('nan')
ks_crit_5pct = float(1.358 / np.sqrt(n)) if n else float('nan')

summ = dict(
    ticker='AMR', market='AE',
    ...
    pit_hist=pit_hist, pit_ks=ks, pit_ks_crit_5pct=ks_crit_5pct,
    pit_uniform=bool(ks < ks_crit_5pct),
    w90_ratio=float((r['w90'] / r['w90_b']).mean()),
    market_gate=dict(verdict=MARKET_VERDICT, skill=reg['market_skill'],
    ...
    first_session=str(df['Date'].iloc[0].date()), last_session=str(df['Date'].iloc[-1].date()),
    max_abs_log=float(scr['max_abs_log']), jump_threshold=float(jump_threshold('AE')),
...
print(f"  cov50 {r['in50'].mean():.2f} cov80 {r['in80'].mean():.2f} cov90 {r['in90'].mean():.2f} "
      f"| PIT mean {pit.mean():.3f} KS {ks:.3f} (5% crit {ks_crit_5pct:.3f}) | "
      f"pit_hist {pit_hist} | w90 ratio {summ['w90_ratio']:.3f}")
print(f"  market gate: {MARKET_VERDICT} {reg['market_skill']:+.4f} CI {reg['market_ci90']}")
