"""SAVOLA — walk-forward backtest reported over three window sets, so the
...
r_all = backtest_v3(df, SA, horizon_months=3, nu=NU, width_cal=CAL,
...
res = dict(
    ...
    production=score(apply_breaks(r_all, SA), 'production window set (post-break origins)'),
)
