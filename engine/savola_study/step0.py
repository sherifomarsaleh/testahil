"""SAVOLA study — Step 0.0 data-quality gate + Step 0 calibration gate.

Production-identical per the engine-reconciliation rule: the live SA fit is read
...
robust_verdict across bootstrap blocks {2,3,4}). SAVOLA is NEW coverage: it has
...
be exactly the committed SA production fit, and the market-panel gate verdict
must match the committed registry (SA: PARITY — the standing market-panel gate
treats PASS and PARITY as proceed, FAIL as stop).
"""
import sys, os, json
...
assert reg['market_verdict'] in ('PASS', 'PARITY'), reg['market_verdict']
assert 'SAVOLA' not in reg['panel_names'], "SAVOLA already covered — not new coverage"

# ---- Step 0.0 — data-quality gate ----------------------------------------
df_raw = raw_load(os.path.join(HERE, '../raw_ohlc/SA/SAVOLA.csv'))
df, dq_log = clean_ohlc(df_raw, 'SAVOLA', verbose=True, market='SA')
scr = screen(df)
