...
def main():
    ...
    print("\nCRITERIA 2, 3, 5 and 6 ARE NOT MEASURED HERE AND ARE NOT REPORTED AS "
          ...
          "     priced exception — MEASURED since 07-09-2026 by\n"
          "     engine/method_reassessment/criterion2.py, which needs ONE NAMED\n"
          "     ADAPTER PER RUN because the five commit forward_ranges.json in five\n"
          "     shapes; run it for the figure. Its second half (claimed corrections\n"
          "     reconcile) IS gated by check_corrections_applied.\n"
          "  3  CORRECTED TWICE ON 07-09-2026. First: this line NAMED THE WRONG\n"
          "     SCORE and a 2027 date was read off it and reported for the WHOLE\n"
          "     criterion. [R-VCAL-01] fixes TWO series; the criterion opens on\n"
          "     the CONTEMPORANEOUS one, which score.py's own docstring calls\n"
          "     'measurable the moment a vintage exists'.\n"
          "     Second, per instruction — PHASE 1 IS THE BACKTEST. Clauses G, B,\n"
          "     C and F score the method against history and GATE Phase 1;\n"
          "     clauses D and E score the LIVE FORWARD record and are Phase 2b's\n"
          "     subject, REPORTED here with their maturity date and holding\n"
          "     nothing. The plan's own Part D says so — 2b 'grades ONLY claims\n"
          "     struck after 2a closed', and every vintage now held was struck\n"
          "     BEFORE it, so D as a Phase 1 gate demanded evidence 2b itself\n"
          "     declares inadmissible. THE COST IS STATED: Phase 1 closes with no\n"
          "     evidence the house lean is INFORMATION rather than merely a lean,\n"
          "     which is exactly what 2b exists to supply.\n"
          "     THIRD, 08-09-2026, per instruction — [R-VCAL-02 CLAUSE THREE]:\n"
          "     clause A is REPORTED rather than gating, and clause G takes its\n"
          "     place — no company called expensive by more than 10% without an\n"
          "     audit behind it. A SYMMETRIC ZERO-BIAS TEST PENALISES A METHOD FOR\n"
          "     DOING THE THING IT IS FOR, and this house exists to find companies\n"
          "     that are cheap. This line is stated HERE as well as in criterion3\n"
          "     so the publish block and the criterion cannot disagree about what\n"
          "     Phase 1 is — which is the whole reason the division is written\n"
          "     twice, and the one thing that makes writing it twice safe is that\n"
          "     they are edited in the same commit.\n"

          "     MEASURED clause by clause by criterion3.py; run it for the six\n"
          "     verdicts, the phase split and what is actually blocking.\n"
