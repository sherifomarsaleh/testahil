"""ADNOCDRILL study — Step 0.0 data-quality gate + Step 0 calibration gate.

Production-identical, following the SWDY model study exactly: the live AE fit is
read from engine/fitted_configs.json (never quoted from memory or from a
document), every production transform is applied (data_quality.clean_ohlc ->
backtest_v3 on the calendar 3-month horizon -> apply_breaks -> scale-normalised
crps/spot -> robust_verdict across bootstrap blocks {2,3,4}).

ADNOCDRILL is NEW coverage: it has no committed fitted_configs entry to
reconcile a verdict against, so the reconciliation ASSERT here is on the FIT
ITSELF — the (nu, width_cal) used must be exactly the committed AE production
fit, and the market-panel gate verdict must match the committed registry.
"""
import sys, os, json
...
assert (NU, CAL) == (float(AE.nu), float(AE.width_cal)), \
    f"registry ({NU},{CAL}) != profile ({AE.nu},{AE.width_cal}) — mirror out of sync"
MARKET_VERDICT = reg['market_verdict']
assert 'ADNOCDRILL' not in reg['panel_names'], "ADNOCDRILL already covered — not new coverage"

# ---- Step 0.0 — data-quality gate ----------------------------------------
df_raw = raw_load(os.path.join(HERE, 'ADNOCDRILL_Stock_Price_History.csv'))
df, dq_log = clean_ohlc(df_raw, 'ADNOCDRILL', verbose=True, market='AE')
scr = screen(df)
...
print(f"Step 0.0: raw {len(df_raw)} rows -> clean {len(df)} rows | "
      ...
      f"(AE artifact threshold {jump_threshold('AE'):.3f}) | flat_frac {scr['flat_frac']:.3f}")

# ---- Step 0 — calibration gate (calendar 3m, carry-anchored, live AE fit) --
r = backtest_v3(df, AE, horizon_months=3, nu=NU, width_cal=CAL,
...
# ---- 1-month horizon, same machinery (the monthly metronome window) --------
r1 = backtest_v3(df, AE, horizon_months=1, nu=NU, width_cal=CAL,
                 use_signal=AE.signal_active, n_paths=20000, seed=42,
                 min_history=260)
n_all_1 = len(r1)
r1 = apply_breaks(r1, AE)
r1['crps_n'] = r1['crps'] / r1['spot']
r1['crps_b_n'] = r1['crps_b'] / r1['spot']
skill1 = 1 - r1['crps_n'].sum() / r1['crps_b_n'].sum()
verd1, detail1 = robust_verdict(r1['crps_n'].values, r1['crps_b_n'].values)
pit1 = r1['pit'].values
pit_hist1 = np.histogram(pit1, bins=10, range=(0, 1))[0].tolist()

summ = dict(
    ticker='ADNOCDRILL', market='AE',
    ...
    h1=dict(windows_scored=int(len(r1)), windows_prebreak_dropped=int(n_all_1 - len(r1)),
            skill_norm=float(skill1), verdict=verd1,
            ci_blocks={str(b): [float(detail1[b][0]), float(detail1[b][1]), detail1[b][2]]
                       for b in (2, 3, 4)},
            cov50=float(r1['in50'].mean()), cov80=float(r1['in80'].mean()),
            cov90=float(r1['in90'].mean()), pit_mean=float(pit1.mean()),
            pit_hist=pit_hist1),
    market_gate=dict(verdict=reg['market_verdict'], skill=reg['market_skill'],
                     ...
                     signal_active=bool(reg['signal_active'])),
    ...
    artifact_threshold=float(jump_threshold('AE')),
    first_date=str(df['Date'].iloc[0].date()), last_date=str(df['Date'].iloc[-1].date()),
    last_close=float(df['Close'].iloc[-1] if 'Close' in df.columns else df['Price'].iloc[-1]),
)
r.to_csv(os.path.join(HERE, 'backtest_rows.csv'), index=False)
r1.to_csv(os.path.join(HERE, 'backtest_rows_1m.csv'), index=False)
with open(os.path.join(HERE, 'step0_result.json'), 'w') as f:
    json.dump(summ, f, indent=1)
print(f"\nStep 0 (3-month): {len(r)} post-break windows ({n_all - len(r)} pre-break dropped) | "
      f"nu={NU} cal={CAL}")
...
print(f"\nStep 0 (1-month): {len(r1)} windows | skill {skill1:+.4f} | verdict {verd1} "
      f"| PIT mean {pit1.mean():.3f} | pit_hist {pit_hist1}")
