...
const TICKERS = {
  ...
  // 17 Aug 2026 — five readings, one field, AED 3.46 to AED 6.21, weighted central 4.92 against a close of 5.94. ADNOC Drilling is a single-customer contract driller: every rig works for ADNOC Onshore, ADNOC Offshore and their affiliates, and the controlling shareholder is the same group. It reports in US dollars and trades in dirhams, so the valuation runs in dollars and converts at the 3.6725 peg only at the last step. Revenue is built BOTTOM-UP from five rig classes on their own counts and their own realised rates — Abu Dhabi onshore, regional onshore, jack-up, island and oilfield services — and OILFIELD SERVICES CARRIES TWO DISCLOSED RIG POPULATIONS, not one: the integrated fleet AND the rigs given at least one discrete service. The one-driver build the first edition used is refuted by the company's own numbers, which imply an integrated rate of MINUS $6.8m a rig. The unit build is reconciled to the company's FY2026 guidance BY SEGMENT (onshore -10.0%, offshore +2.5%, oilfield services +8.0%) rather than at group, where the same two errors were cancelling inside 1.9%; the rates therefore set the growth path and the guidance sets the FY2026 level. The two 2026 business combinations are consolidated on BOTH sides of the balance sheet from a note-5 entry that closes to zero against owners' equity. THE CRUX IS THE TERMINAL QUESTION: Abu Dhabi's production-capacity target is met in 2027 and the customer has not extended the programme beyond it, so the study computes BOTH futures in full — continued expansion 6.21, capacity plateau 5.40 — and publishes them as separate lines rather than averaging them into one. Terminal value is 76.0% and 72.3% of enterprise value respectively, a stated line of the bridge. Cost of capital 8.01% on a tier-1 own-stock weekly beta of 0.795 measured against the published FTSE ADX General Index; weights on GROSS debt. The minority is deducted ONCE, through the put liability the company recognised over it, because it has already charged a matching investment reserve against owners' equity. Enterprise value and the bridge are dated the same day: EV rolled from 31-Dec-2025 to 30-Jun-2026 at the cost of capital, less the free cash flow actually generated, then accreted 38 days to the price anchor.
    ...
    asof: {
      ...
      tech: { data:"2026-08-07", computed:"2026-09-01" }
    },
    files: {
  ...
  SAVOLA: {
    ...
    asof: {
      ...
      tech: { data:"2026-08-18", computed:"2026-09-01" }
    },
    files: {
  ...
  PHAR: {
    ...
    asof: {
      ...
      tech: { data:"2026-08-06", computed:"2026-09-01" }
    },
    files: {
  ...
  ARCC: {
    ...
    ccy: "EGP",
    fair: { bear: 48.47, base: 52.90, full: 59.50 },      // 1 Sep 2026, revision 4 \u2014 four-lens weighted central EGP 52.90 against the latest known close of 59.00 (\u221210.3%), which TRIPS [R-GAP-01] and is audited in engine/arcc_study/GAP_REVIEW_01-09-2026.md under all eight headings. Weights 50/20/22/8: FCFF DCF 55.21 / relative EV\u2044EBITDA 45.65 / normalised earnings power 49.64 / asset\u2044replacement cost 65.57. THREE THINGS MOVED IT FROM REVISION 3\u2019S 54.65, each measured on the lens it actually moves. (1) THE BETA, worth \u22125.35 and the largest single cause: revisions 1\u20133 carried 0.6281 from a regression against an EQUAL-WEIGHT COMPOSITE of the other covered Egyptian names, which SIGCM clause 6 calls a hard fail rather than a tier \u2014 a basket of covered names explains a covered name better because it partly consists of companies like it, so the better R\u00b2 was the artefact. Re-derived through beta_regression.own_stock_beta() against the EGX30 it returns 0.698 on R\u00b2 0.047, BELOW the 5% usability floor, so tier 1 is unavailable and the study falls to a tier-2 same-country peer median of 0.9275 (Lecico 0.815, Egypt Aluminium 0.919, Orascom Construction 0.936, Egyptian Chemical 1.030). Peer leverage is not sourced so the unlever/re-lever step is NOT done and is flagged; ARCC is net cash and its peers carry debt, so doing it could only LOWER the beta and RAISE value \u2014 the adopted figure is the conservative end. (2) THE BRIDGE, worth \u22123.13: it now stands on the DISCLOSED 30-June-2026 balance sheet (cash 1,970.501 less interest-bearing debt 1,283.288 = net cash 687.213) instead of the roll-forward revision 3 had to build, which came out EGP 1,173mn too generous because it could not see a 698.6mn inventory build, an 832.8mn rise in debtors and 608.4mn of capex in six months. (3) THE REVIEWED HALF, worth +6.73: the H1-2026 interim accounts (filed 13 Aug 2026, a week AFTER revision 3\u2019s valuation date \u2014 new information, not a missed source) show revenue +10.6% and a 40.5% gross margin, with local goods +18.5% against export goods \u22123.8%. Price, cost AND services are calibrated on the SAME half so the margin stays an OUTPUT; calibrating price alone would have manufactured a margin, which is [L-009]. HOW MUCH OF A YEAR THE FIRST HALF IS was measured on every half the archive supports \u2014 44.2% / 45.5% / 52.7% \u2014 and is NOT stable, so the MEDIAN is used and the range published; an earlier cut of this revision grossed up on FY2025 alone, reached 61.15, and was wrong to rest a 12.7% lift on one year. Carries this name\u2019s own FUNDAMENTAL WALK-FORWARD (engine/arcc_walkforward, [R-FCAL-01]): twelve fiscal years FY2014\u2013FY2025, eight origins, 25 scoreable cells. The method BEATS both naive benchmarks on net profit (+0.434 vs freeze, +0.578 vs trend) \u2014 the first name in this repository where it does. Twelve correction candidates cleared the statistical bar; NINE then failed their own out-of-sample test, which is a specification problem no multiplier may hide; ONE adopted (manufacturing depreciation \u00d71.0298). Years 3\u20135 are published as RANGES from that record. Terminal value 48.8% of EV; terminal growth 5% against a terminal risk-free rate built from 5% inflation, i.e. 0% real. full = weighted bull central.
    dist: {
  ...
  SWDY: {
    ...
    asof: {
      ...
      tech: { data:"2026-08-05", computed:"2026-09-01" }
    },
    files: {
  ...
  CLHO: {
    ...
    asof: {
      ...
      tech: { data:"2026-08-23", computed:"2026-09-01" }
    },
    files: {
  ...
  DEWA: {
    ...
    asof: {
      ...
      tech: { data:"2026-08-21", computed:"2026-09-01" }
    },
    files: {
  ...
  LULU: {
    ...
    asof: {
      ...
      tech: { data:"2026-08-27", computed:"2026-09-01" }
    },
    files: {
...
const CALIB = {
  ...
  "NSE:RELIANCE": {w: 57, in50: 56, in90: 93, through: "2026-04-29"},
