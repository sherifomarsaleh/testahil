...
const TICKERS = {
  CLHO: {
    ...
    dist: {
      // PRIMARY cone = the FULL-history regime (every devaluation Egypt has actually delivered).
      // This is the honest default; the calmer cone is shown BESIDE it, never instead of it.
      t20: { label:"1 month (T+20)",  p5:13.55, p25:15.44, p50:16.55, p75:17.74, p95:20.19, resolve:"2026-08-09" },
      t60: { label:"3 months (T+60)", p5:11.87, p25:15.02, p50:17.02, p75:19.29, p95:24.36, resolve:"2026-10-04" },

      // TWO REGIMES, BOTH PUBLISHED (13-Jul-2026). Egypt is a market where political turmoil,
      // regional geopolitics, sudden devaluation and rate spikes are the ORDER OF THE DAY, not
      // the exception. A single cone silently picks one answer to a question with two legitimate
      // answers, so we fit and publish BOTH:
      //   full    -> "what if Egypt keeps doing what Egypt does?"  (every crisis since 2016 in the sample)
      //   current -> "what if the current calm holds?"             (from 1-Apr-2024, post the March-2024 float)
      // THEY AGREE ALMOST EXACTLY IN THE MIDDLE AND DIVERGE IN THE TAIL, which is the whole point:
      // a devaluation is a tail event. Same median (17.02). But the 99% band is 21% wider under the
      // full regime, and a >50% crash is TWICE as likely. That spread is the price of assuming the
      // shocks are over -- and it is exactly what one blended cone would have destroyed.
      regimes: {
        full: {
          label:"Full history (2016 \u2192 today)", primary:true,
          blurb:"Every devaluation, rate spike and shock Egypt has actually delivered since the 2016 float \u2014 Mar-2022, Oct-2022, Jan-2023 and the big Mar-2024 float. This is the cone if Egypt keeps behaving like Egypt. Wide on purpose.",
          nu:4.0, width_cal:0.958, windows:508,
          verdict:"FAIL", skill:-0.0382, market_verdict:"PASS", market_skill:0.0169,
          t20:{ p1:11.66, p5:13.55, p25:15.44, p50:16.55, p75:17.74, p95:20.19, p99:23.5, above_spot:0.558 },
          t60:{ p1:9.0, p5:11.87, p25:15.02, p50:17.02, p75:19.29, p95:24.36, p99:31.96, above_spot:0.594 },
          crash:{ dn20:0.0958, dn30:0.0388, dn40:0.0155, dn50:0.0063 }
        },
        current: {
          label:"Current regime (Apr 2024 \u2192 today)", primary:false,
          blurb:"From 1 April 2024, after the March-2024 float. The pound still fluctuates and rates still move, but there has been no step-devaluation since. This is the cone if the current calm holds \u2014 narrower, and conditional on an assumption Egypt has repeatedly falsified.",
          nu:5.0, width_cal:0.85, windows:232,
          verdict:"PARITY", skill:0.0017, market_verdict:"PASS", market_skill:0.0388,
          t20:{ p1:12.21, p5:13.77, p25:15.49, p50:16.55, p75:17.68, p95:19.88, p99:22.49, above_spot:0.56 },
          t60:{ p1:9.8, p5:12.22, p25:15.11, p50:17.02, p75:19.18, p95:23.68, p99:29.64, above_spot:0.598 },
          crash:{ dn20:0.0831, dn30:0.03, dn40:0.0099, dn50:0.0032 }
        }
      }
    },
...
const LEDGER = [
  ...
  { instrument:"CLHO", asset_class:"equity", anchor_date:"2026-07-12", anchor_price:16.31, ccy:"EGP",
    ...
    note:"TWO REGIMES, BOTH PUBLISHED. Egypt is a market where political turmoil, regional geopolitics, sudden devaluation and interest-rate spikes are the ORDER OF THE DAY, not the exception \u2014 so a single cone would have to silently pick one answer to a question with two legitimate answers. We fit and publish both. FULL HISTORY (2016\u2192, 508 windows, nu=4.0, cone 0.958): every devaluation Egypt has actually delivered \u2014 Mar-2022, Oct-2022, Jan-2023, and the big Mar-2024 float. Panel PASS +1.7%%. On THIS regime CLHO is a name-level FAIL (scale-normalized CRPS skill \u22123.8%%, CI entirely below zero and robust across bootstrap block sizes): once every crisis is in the sample, our cone does NOT beat a carry-anchored random walk on this name. We publish that rather than bury it. CURRENT REGIME (1-Apr-2024\u2192, 232 windows, nu=5.0, cone 0.850): after the March-2024 float; the pound still moves but there has been no step-devaluation. Panel PASS +3.9%%; CLHO recovers to PARITY (+0.2%%). THE TWO AGREE ALMOST EXACTLY IN THE MIDDLE AND DIVERGE IN THE TAIL, which is the entire point \u2014 a devaluation IS a tail event. Identical median (17.02). But the 99%% band is 21%% wider under the full regime, and a fall of more than 50%% is TWICE as likely (0.63%% vs 0.32%%). That spread is the price of assuming Egypt\u2019s shocks are over. Neither cone is \u2018the\u2019 answer and we do not blend them: averaging would destroy the only thing worth knowing. The cone is a 3-month object and is NEVER blended with the 12-month fundamental anchor. See study \u00a73 and the Calibration Ledger.",
