...
const TICKERS = {
  ...
  ARCC: {
    ...
    ccy: "EGP",
    fair: { bear: 48.75, base: 54.10, full: 59.53 },      // 1 Sep 2026, revision 4 \u2014 four-lens weighted central EGP 54.10 against the latest known close of 59.00 (\u22128.3%). Weights 50/20/22/8: FCFF DCF 57.60 / relative EV\u2044EBITDA 45.65 / normalised earnings power 49.64 / asset\u2044replacement cost 65.57. MOVED FROM REVISION 3\u2019S 54.65 BY SIX LEGS, each priced on the lens it actually touches: beta \u22124.27, the bridge \u22123.44, the reviewed half +5.95, the capex anchor +0.84, the disclosed other-income line +0.42, the pound-equivalent cost of debt \u22120.06. THE BETA is the largest: revisions 1\u20133 carried 0.6281 from a regression against an EQUAL-WEIGHT COMPOSITE of the other covered Egyptian names, which SIGCM clause 6 calls a hard fail \u2014 a basket of covered names explains a covered name better because it partly consists of companies like it, so the better R\u00b2 was the artefact. Against the EGX30 it returns 0.698 on R\u00b2 0.047, BELOW the usability floor, so tier 1 is unavailable and the study falls to a tier-2 same-country peer median of 0.9275; peer leverage is not sourced so the unlever/re-lever step is flagged undone, and doing it could only LOWER the beta and RAISE value. THE BRIDGE now stands on the DISCLOSED 30-June-2026 balance sheet (net cash 687.213) instead of a roll-forward that was EGP 1,290mn too generous. THE REVIEWED HALF (H1-2026, filed 13 Aug 2026 \u2014 a week AFTER revision 3\u2019s valuation date, so new information rather than a missed source) is calibrated into price, cost AND services together so the margin stays an OUTPUT; how much of a year the first half is was measured on every half the archive supports (44.2 / 45.5 / 52.7%) and the MEDIAN used, because it is not stable. A GAP REVIEW WAS REQUIRED AT \u221210.3% AND IS PUBLISHED (engine/arcc_study/GAP_REVIEW_01-09-2026.md). It found three further valuation defects that the eight headings alone would not have: the SENSITIVITY FUNCTION discounted the terminal value on a different factor from the headline and was quoting every alternative 3.7% high \u2014 one document, two models; the CAPEX anchor was set at the MIDDLE of a band the study had just said was inflated by growth spending, so above both observed years, and is reset to the most recent full year\u2019s total of USD 3.23/t; and a DISCLOSED other-income line (export subsidy at 0.856% of export revenue, note 7) was registered and consumed by nothing, which is [L-018]. The separate EGP 467.8mn subsidy collected in H1-2026 is left in cash and PRICED as a scenario (at 5% of export revenue the cash-flow lens is 59.95). Carries this name\u2019s own FUNDAMENTAL WALK-FORWARD (engine/arcc_walkforward, [R-FCAL-01]): FY2014\u2013FY2025, eight origins, 25 cells. The method BEATS both naive benchmarks on net profit (+0.434 vs freeze, +0.578 vs trend) \u2014 the first name here where it does. Twelve correction candidates cleared the statistical bar; NINE then failed their own out-of-sample test, which is a specification problem no multiplier may hide; ONE adopted. Years 3\u20135 published as RANGES from that record. Terminal value 47.4% of EV; terminal growth 5% against a terminal risk-free rate built from 5% inflation, i.e. 0% real. full = weighted bull central.
    dist: {
