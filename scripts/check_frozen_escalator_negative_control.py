...
EXPECTED_CASES = 11        # asserted against the declared constant
...
def m_no_lines_probed(d):
    """Directories present, every line renamed so nothing can be probed. The
    distinction an absent answer hides behind [R-ENF-04]."""
    p = os.path.join(d, GATE)
    s = _read(p)
    old = '("AMOC", "amoc_walkforward", "cpi_path", ["net_sales", "cost_of_sales"]),'
    if old not in s:
        return False
    s = s.replace(old, '("AMOC", "amoc_walkforward", "cpi_path", ["no_such_line"]),', 1)
    s = s.replace('("EGCH", "egch_walkforward", "cpi_path", ["revenue", "cost_of_sales"]),',
                  '("EGCH", "egch_walkforward", "cpi_path", ["no_such_line"]),', 1)
    s = s.replace('("ARCC", "arcc_walkforward", None, ["revenue", "cogs"]),',
                  '("ARCC", "arcc_walkforward", None, ["no_such_line"]),', 1)
    _write(p, s)
    return "no_such_line" in _read(p)
